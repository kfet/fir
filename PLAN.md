...
Port the ACP (Agent Client Protocol) mode from `../pi-mono-acp`. This exposes fir as an ACP-compliant
...
**Key facts:**
- ACP is a standalone mode (`pkg/modes/acp/`), NOT an extension — the upstream analysis was for TS runtime extensions, but the same 5 architectural gaps apply to fir's compiled-in Go extensions (see `AGENTS.md`)
