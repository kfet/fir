...
func TestParseArgs_SessionNameShort(t *testing.T) {
	args := ParseArgs([]string{"-n", "quick"})
	if args.SessionName != "quick" {
		t.Errorf("expected session name from -n, got %q", args.SessionName)
	}
}
