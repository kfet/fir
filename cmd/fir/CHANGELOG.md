...
- Poe `assistant` bot (backed by `gpt-5.2-chat-latest`) no longer 400s with `Unsupported value: 'low' is not supported with the 'gpt-5.2-chat-latest' model. Supported values are: 'medium'.` when the user's thinking level is anything other than medium. This specific Poe bot, unlike every other reasoning bot in the catalog, accepts only `"medium"` for `reasoning.effort` — not the usual `{low, medium, high, xhigh}` enum. `buildOpenAIRequestBody` now clamps the effort to `"medium"` when `provider == poe && id == "assistant"`, so `/think low` / `/think high` on that bot is a silent no-op instead of an API error.
