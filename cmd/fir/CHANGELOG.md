...
- Extension frontmatter gained a `tools:` key. It enumerates the tool names the extension registers and forces eager startup so the tool surface is available without waiting for an event to fire. Required when an extension subscribes to events (which would otherwise defer it to lazy startup) and also exposes tools — without `tools:` declared, the tools wouldn't be registered until the first matching event arrived. The frontmatter completeness test now flags drift in both directions (registered-but-undeclared and declared-but-unregistered) and `fir frontmatter --fix` rewrites the line in sync with the init handshake. Builtin extensions `agent-introspect`, `aside`, `autoresearch`, `demo`, `doctor`, and `install` updated to declare their tools.
- `aside` extension now subscribes to `session_start` and prepends a `[SYS_EXT]` note teaching the LLM *why* the advisor model exists when one is configured. Principle-based, not a checklist — the note frames the advisor as a second opinion to reach for when the LLM's own reasoning (not its tools) is the bottleneck and the cost of being wrong outweighs the cost of asking, plus a "earlier is cheaper" nudge to encourage front-loaded planning escalations rather than mid-flow rescues. Co-located with the tool that owns the behaviour and only fires when the advisor is enabled, so users who run `/aside-advisor off` see zero prompt bloat.
