...
- Anthropic provider: do not drop empty/whitespace-only sibling text blocks from an assistant message that contains a signed thinking block. Anthropic's API rejects the next request with `400 messages.N.content.M: thinking or redacted_thinking blocks in the latest assistant message cannot be modified` when *any* block in that turn is changed (including dropping a sibling). `convertAnthropicMessages` now detects the presence of a signed/redacted thinking block and preserves all sibling blocks verbatim. Fixes the regression at the root in `pkg/ai/providers/anthropic.go`, so it applies to every mode (CLI, ACP, headless). Comprehensive coverage added in `pkg/ai/providers/anthropic_thinking_invariants_test.go`: a 10-case block-sequence matrix (incl. the observed `messages.1.content.9` shape, redacted-only, redacted+signed, interleaved thinking + tool_use), a multi-turn conversation property test, a same-provider-different-model-ID guard, an SSE-stream→replay roundtrip for both `thinking` and `redacted_thinking`, a tool_use input byte-stability test, and a negative test confirming the empty-text filter still applies when no thinking is present. The agent's partial-message `hasContent` check (`pkg/agent/agent.go`) also now counts redacted/signed thinking blocks even when their text is empty, so a redacted-only assistant turn is no longer silently dropped.
