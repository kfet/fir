...
### Changed
- **One cache location and one collection policy for everything fir extracts out of its own binary.** Builtin skills, builtin extensions and the extension SDKs each answered "where does an extracted tree live, and who deletes it" differently: two hardcoded `~/.cache` paths that ignored `$XDG_CACHE_HOME`, one `$TMPDIR` path, and no collection anywhere. New `pkg/cache` is the single answer — `cache.Dir(sub)` resolves `$XDG_CACHE_HOME/fir/<sub>` when set and `~/.cache/fir/<sub>` otherwise, and `cache.SweepAged` ages unclaimed trees out. Honouring one half of the XDG spec and hardcoding the other was an oversight, not a policy: on a host that sets both, fir read its config from where it was told and then wrote hundreds of megabytes where it was not.
