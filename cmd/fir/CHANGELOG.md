...
### Added
- **The coverage-gate on-ramp is cleared: queue items 1-6 sealed at 100%.** `pkg/extension/apikind`, `pkg/ai/envkeys`, `pkg/extension/sdk`, `pkg/ai/providers/declcfg`, `pkg/agent/tools` and `pkg/log` are out of section 2 of `.covignore` — the "pure debt" half of the ledger, which exists only to be deleted — and the gate now enforces them forever. With `pkg/pkg` (queue item 12, taken out of order) that is **seven packages promoted, 372 → 1,483 gated statements**, and the whole tree from 67.8% to 68.7%; `COVERAGE_FLOOR` moves 66 → 67, keeping the ~1 point of slack the Makefile comment argues for. **Nothing was added to the ledger to achieve it**: every uncovered branch was either reached by a test or deleted as genuinely dead code, and no test skips, mocks the thing under test, or touches the network. The tests are aimed at the properties that would actually cost something if they regressed rather than at the statement count — the `apikind` registry under concurrent provider `init()`s (a plain nil on a miss, not a typed nil that survives an `h != nil` check); `envkeys`' Vertex ADC gate failing closed on each of its three preconditions and `KnownApiKeyEnvVars` asserted as a property against the live provider registry rather than a list that rots; `declcfg`'s env-access refusal (the reason a declarative provider config cannot exfiltrate process environment) and its float64-renders-as-"4096" rule; the schema codec's tolerance rules (undeclared keys and unknown enum tokens pass through, which is what keeps old transcripts rendering) and its located structural errors; and every failure path of log rotation, where the contract is that no bytes are lost, no partial `.gz` is left looking like a valid backup, and the rotating flag is always cleared. Three seams were introduced, each in the style already used by the file it lives in: `sdkFS` (an `fs.FS` var beside the existing `cacheDir` var) because `embed.FS` cannot fail a read; `debugEnvSet()` split out of `pkg/log`'s `init()` because `init` runs once per process before any test can set `FIR_DEBUG`; and nothing else. Two dead branches were deleted rather than covered: `fnRandID` checked an error from `crypto/rand.Read`, which since Go 1.24 never returns one (it fills the buffer or crashes the program), and `globPatterns`' `WalkDir` error check whose callback swallows everything. Note a standing constraint the gate now enforces: three of these packages force failures with `chmod 0500`, so **the suite must run unprivileged** — as root those checks are no-ops and the build fails.
- **`pkg/pkg` is now at 100% statement coverage and sealed behind the gate** — the first package promoted out of the `.covignore` debt ledger since the gate was adopted, taking the gated scope from 372 to **890 statements** (518 of them `pkg/pkg`'s). The package manager was 69.6% covered, and the gap was concentrated exactly where it matters: `resources.go` at 27.8% (nothing exercised the `fir.json` manifest path at all — glob expansion, trailing-slash directory patterns, bad-pattern rejection), plus every error path in the git plumbing. All git tests run against **local bare repos created in `t.TempDir()`**, with an `insteadOf` rewrite standing in for `https://github.com/`, so no test touches the network and none is skipped when offline. The harder branches are pinned by real failures rather than mocks: a sparse checkout that fails mid-way must delete its half-built clone (forced with a cone-mode-invalid subdirectory), a read-only working tree makes `sparse-checkout disable` fail, an unwritable `.git/info` makes `sparse-checkout set` fail, and `Pull`'s error precedence — fetch error reported *only* when the fast-forward could not stand in for it, the rule that keeps a racing sibling fetch from becoming a spurious failure — is pinned by diverging a clone from a moved origin with a broken remote URL. Five dead or duplicated fragments were removed rather than covered: the unused `gitDirBase` helper, the unused exported `CurrentRef`, the unconditionally-`nil` `error` returns of `addPackage`/`removePackage` (`SettingsBackend.Set*` returns nothing, so persistence failures were already invisible — the signature was aspirational plumbing, and the two `if err != nil` branches at the call sites were unreachable), a duplicated local-path resolution in `ParseSource` (now one `parseLocal`), and a `filepath.WalkDir` error check whose callback swallows every error unconditionally. **No line-level or file-level exclusion was added** — the ledger's own rule is file-level patterns only, and every branch was either reached by a test or deleted as dead.

### Changed
- **ACP mode now builds against `acp-go-sdk` v0.13.5, up from v0.6.3.** The gap was not merely staleness: v0.6.3 predates the SDK's `extensions.go` entirely, so `NotifyExtension` and `ExtensionMethodHandler` — the plumbing for ACP's spec-blessed `_`-prefixed custom methods and notifications — were unreachable from fir at all. **The wire is unchanged throughout.** fir keeps its own dispatch table (`rawMethodHandler`), so where upstream deleted a type fir still speaks, the type is reproduced locally rather than the JSON being altered: `legacymodels.go` reinstates `SessionModelState`/`ModelInfo`/`ModelId` and the `session/set_model` request pair that v0.13 replaced with generic config options, and the legacy `models` object is re-attached to `session/new` beside the `configOptions` injection already there. That shape is still what clients read — `acp-kit` parses both generations — and migrating to config options is a separate change with model-dropdown consequences. `AuthMethod` became a union; fir emits the `Agent` variant, which marshals with no top-level `"type"` and is therefore byte-identical to the flat pre-v0.13 shape, with fir's extended auth kinds staying in `_meta` where its clients already look for them. The four `Agent` methods the spec gained are implemented rather than stubbed: `CloseSession` delegates to the teardown already behind the older unstable `session/release` (identical contract — cancel in-flight work, free the session — so the two entry points share one implementation); `ListSessions`, `ResumeSession` and `SetSessionConfigOption` adapt the spec-shaped types onto fir's existing handlers, which are renamed `*Local` and continue to serve fir's own dispatch route verbatim, including the `"title": null` that fir always emits and the spec marks `omitempty`. `Logout` reports method-not-found deliberately: fir's credentials are process-wide (`pkg/auth` backs the CLI, the TUI and every other mode), so honouring a session-scoped logout would sign the user out everywhere with no way to confirm — use `fir auth logout`. Also renamed by upstream: `KillTerminalCommand` → `KillTerminal`, and `_meta` fields are now typed `map[string]any` rather than `any`, removing a type assertion at each use. Newly available to build on, unused so far: the `UsageUpdate` (context window and cost) and `SessionInfoUpdate` (session title, timestamps) session-update variants, both standard replacements for parts of what fir currently hand-rolls in `_meta`.

- **Model catalog regenerated**: added `opencode-go/union-alpha`, `opencode/union-alpha` (+2 on aggregators); removed 6 aggregator model(s); 24 model(s) with changed pricing/limits.

### Fixed
- **fir leaked a complete SDK copy into `~/.cache/fir/sdks/` every time two processes raced to extract it.** `EnsureExtracted` publishes the embedded SDK tree by renaming a temp directory into its content-addressed home. When a second fir process gets there first the rename fails with ENOTEMPTY, and the loser correctly adopts the winner's copy — but it also set `success = true` on the way out, with the comment "prevent cleanup of already-renamed dir". The rename had *failed*, so nothing had been renamed: the only thing that flag suppressed was the deferred cleanup of our own temp directory, which was then abandoned as `.extract-XXXXXX` forever. The concern the comment names could not happen either, since the deferred `RemoveAll` only ever touches `tmp`, never the published directory. Every lost race leaked one full copy; on a host running several fir sessions the cache grew without bound and nothing ever collected it. Leaving `success` false does the right thing. Found by asserting that no `.extract-*` directory survives a simulated lost race — the assertion, not the error path, is what caught it.
- **`containsPackage` recognised an unparseable settings entry only through a branch that could never execute.** Its verbatim-match fallback (`if existing == source`) sat *after* a parse of the same string that had already returned early on failure — so reaching it required one string to both parse and not parse within a single call. `filterPackage` meanwhile does honour a verbatim match when removing, which meant install-dedup and uninstall disagreed about what "the same package" is: a hand-edited or legacy entry fir can no longer parse could be removed by its exact spelling but never recognised as already installed, so re-installing it appended a duplicate. The candidate's identity is now resolved the same way `filterPackage` resolves its target (empty identity when unparseable, which matches nothing, since every real identity is non-empty), and the verbatim check runs before any parse — reachable, tested, symmetric with removal, and one parse cheaper on the common path.
- `findJSTSEntryDirs` no longer carries an error return that could never be non-nil. Every arm of the scan swallows its own `ReadDir` failure (an unreadable directory is skipped, deliberately — one bad subdirectory must not cost a package its other extensions), so the plumbing propagated a value that was always `nil` through five call sites. Deleted rather than covered, matching the `WalkDir`-callback precedent already established in this package.
- The `main → run.sh` wrapper symlink is now replaced **atomically** (temp link + `rename`) and re-pointed only when its target is inside fir's **actual** SDK cache. Previously the re-point was licensed by any ancestor directory merely *named* `sdks`, so a user maintaining their own fork at `~/projects/sdks/myfork/run.sh` would have had their hand-made symlink silently overwritten; and a remove-then-create that lost a race (or died) between the two steps left no `main` at all, making the extension permanently undiscoverable until the next `fir install`. Install-time repair now delegates to the same healing policy discovery uses, so the two paths can no longer disagree.
- `fir install` now generates a `main → run.sh` wrapper for the conventional `extensions/<name>/index.ts` layout. Wrapper generation only looked at `extensions/` itself while package auto-discovery recurses fully, so a JS/TS entry one level down was discovered-but-never-wrapped — i.e. never loaded.
- Package discovery no longer reads an entire compiled-binary entry point into memory to look for a comment frontmatter block that a binary cannot carry; it reads a bounded 64 KiB prefix. It also logs (at debug) when two entry points in one package claim the same extension name, instead of silently letting the last one win.
- Installed JS/TS ("pi-style") and **compiled binary** package extensions are now discovered and loaded by a fresh session. Package discovery previously collected only `.py`/`.sh` files carrying comment frontmatter, so a bare extensionless `main` symlink (or a binary, which can't carry frontmatter at all) was never picked up — a new session loaded nothing (`Discovered: 0 extension(s)`). Package discovery now honours the same extensionless entry-point convention as `.fir/extensions/<name>/`: a `main` (or `<dirname>`) executable in any package directory is an extension named after its directory, with frontmatter optional. `fir install` creates a `main → run.sh` symlink next to each JS/TS entry point (in core, so the `fir install` CLI verb and the `/install` slash-command behave identically); a `main → run.sh` symlink left dangling by an SDK-cache change is self-healed to the current SDK's `run.sh` at discovery time, so the extension keeps loading across fir upgrades. `fir install git:github.com/huggingface/pi-llama` now reports `1 extension(s)`, `fir packages` shows it, and a live session spawns the `pi-llama` extension and completes its handshake.
- `run.sh` (the generic JS/TS runtime wrapper) now finds bun/node even when fir's spawn `PATH` is minimal: it probes common install locations (`~/.bun/bin`, `~/.local/bin`, nvm/fnm/volta/pnpm/deno, `/usr/local/bin`, `/opt/homebrew/bin`). It also detects the `@earendil-works/pi-coding-agent` import scope (the pi-llama fork) as pi-mono, in addition to `@mariozechner/`.
- **A literal `apiKey` from an extension-registered provider now authenticates.** `ProviderSpec` gained an `api_key` field carrying the key *itself*, as distinct from `env_keys.primary` which carries the NAME of an environment variable. The pi-mono compat layer mapped pi's `config.apiKey` — which in pi semantics is the literal value — onto `env_keys.primary`, so a provider like pi-llama's `llama-cpp` (whose default is the conventional `"no-key"` a local llama.cpp server accepts) failed with `no API key for provider "llama-cpp"` and needed a `models.json` stanza or a bogus `LLAMA_API_KEY=<name>` env var to work at all. A literal key is now published to the models registry at the init handshake (`models.RegisterProviderAPIKey`, withdrawn on unregister) and resolved **last** — after the runtime `--api-key` override, an `auth.json` credential, the environment variable and any `models.json` custom-provider stanza — so what an extension ships is always overridable by user configuration and never masks it. The key is held in memory only; it is never written to `auth.json`. It is deliberately kept out of `ModelRegistry.customProviderApiKeys`, which is rebuilt from the `models.json` snapshot on every `Refresh()` and would silently drop it. `pi_compat` now passes `apiKey` as the literal and *additionally* as `envKeys.primary` when it matches `^[A-Z][A-Z0-9_]+$`, so the old env-var-name reading keeps working without a heuristic having to reject anything: if a variable of that name is set, precedence picks it; otherwise the string is the key.
- **The Node SDK answers `auth/resolve_endpoint`.** fir asks every extension to confirm or correct a selected model's endpoint, but the Node SDK had no handler for the method at all, so every session using a pi-registered provider logged `WARN hook call failed … auth/resolve_endpoint: Unknown auth method`. New `authResolveEndpoint(providerId, handler)` registration mirroring the Python SDK's `auth_resolve_endpoint`; an extension with no handler answers `null` ("no correction") rather than `-32601`, since the hook is legitimately optional and fir fans it out to everyone. `pi_compat` installs a resolver for any pi provider configured with a `baseUrl`, reporting that URL when fir has none. Unknown auth methods still error.
- **A package's `main → run.sh` wrapper now follows SDK upgrades even when the old SDK cache survives.** Self-healing previously fired only for a *dangling* symlink (pruned cache dir). If the previous `~/.cache/fir/sdks/<hash>/` directory still existed the link kept resolving, so the extension silently ran a superseded `run.sh`/`fir_ext.js`/`pi_compat.js` — everything looked healthy while the extension was several releases behind, and a freshly built fir would see none of its own SDK's fixes. Discovery now re-points any wrapper symlink whose target is a `run.sh` **inside the SDK cache** at the current SDK; a symlink to a `run.sh` anywhere else is the user's and is left untouched.
- **A pi extension is named after its package directory, not its entry file.** `pi_compat` derived the handshake name from the entry-point filename, so every pi package whose entry is the conventional `index.ts`/`main.ts` reported itself as `index` — fir discovered it as `pi-llama` but then logged, named and addressed it as `index`. The directory name now wins for a generic entry filename; a specific one (`todos.ts`) is still used as-is.
...
### Fixed
- **`/reload` did not pick up a provider you had just logged into.** Authenticate a new provider mid-session — `fir login` in another terminal, a sibling fir session, Claude Code writing the same shared `auth.json` — and the running session stayed blind to it: `/reload` (and the ACP `!reload` surface) rebuilt models, extensions, skills, themes and MCP servers, but `AuthStorage` only ever read the credential file at construction (`newAuthStorage` → `Reload`) and on OAuth token refresh. Nothing on the reload path re-read it, so `HasAuth` kept answering with the process's startup state and `GetAvailable` — which filters models by exactly that — kept omitting the provider. `ModelRegistry.Refresh` therefore did a lot of honest work over a stale credential map, which is why the symptom looked like a model-catalog problem rather than an auth one, and why restarting fir was the only cure. `Refresh` now re-reads the auth storage through its existing flock'd backend *before* rebuilding, which fixes both reload surfaces at once (they already funnel through it) and, as a free side effect, makes a logout performed elsewhere propagate too. Runtime-injected API keys are untouched: they live outside the on-disk map, and a test pins that they survive the re-read. Nothing new is logged — the reload path never sees credential contents, only the provider names `HasAuth` already exposed.

## [1.10.0] - 2026-09-14

### Added
- **`aside` gains agentic delegate mode.** Passing `goal` (with `delegate=true`; mutually exclusive with `tools`) lets the cheap delegate model drive its own **read-only** tool calls — `read`/`glob`/`grep`/`ls`, never anything mutating — in a loop until it can answer, instead of forcing the caller to know the whole chain upfront. Bounded by `max_iterations` (default 8, clamped 1-20), a 200k-char total tool-output cap and a 5-minute wall clock; hitting any cap makes one final toolless synthesis call and names the cap in the result header. `allow_tools` narrows the set and rejects anything outside the read-only four. Escalation deliberately does NOT get agentic mode (the pattern stays "delegate gathers → escalate judges"), and an exhausted delegate chain returns an error telling the caller to work inline rather than silently running the loop on the executor model. Usage is summed across the whole loop and one observable card tracks it per iteration.
- **The full OpenRouter provider-routing surface is now settable from `models.json`.** fir's loader exposed two of thirteen fields (`only`, `order`) while the underlying `github.com/kfet/ai` struct carried the rest, so the knobs that matter most for correctness and privacy were unreachable from config: `requireParameters` (only route to backends supporting every parameter in the request — the vision / reasoning-effort mismatch guard), `zdr` and `dataCollection` (Zero Data Retention routing), `allowFallbacks: false` (fail rather than silently leave the allow-list), plus `ignore`, `quantizations`, `sort`, `maxPrice`, `preferredMinThroughput`, `preferredMaxLatency` and `enforceDistillableText`. Keys are camelCase in config and translated to OpenRouter's snake_case on the wire; the three union-typed fields (`sort`, `preferredMinThroughput`, `preferredMaxLatency`) are forwarded verbatim and validated by the API rather than locally. Purely additive to the `ProviderConfig` wire format, so the published catalog overlay is unaffected.

### Fixed
- **The `aside` tool declared no host-side timeout, so it inherited the 30-second default** while its body waits on `ctx.call_tool(timeout=60)` and runs multi-minute LLM work — a documented SDK invariant violation that only survived because `CallHook`'s deadline is activity-aware and the extension happens to be chatty. It now declares `timeout=-1`; the call stays bounded by turn cancel / ESC and `side_query`'s own 600s idle timeout.
- **51 tests in `pkg/resources/testdata/aside_test.py` never ran.** A stray `if __name__ == "__main__": unittest.main()` block sat in the *middle* of the file, so everything defined after it — the streaming-card, empty-content-retry, redacted-thinking and dynamic-default-chain suites — was never collected when the file was executed as a script. The guard now lives at the end of the file, where it belongs.
- **A per-model routing override silently rewrote the built-in catalog entry it was overriding.** `mergeCompat` shallow-copies the base `OpenAICompletionsCompat`, so the merged struct's `OpenRouterRouting` / `VercelGatewayRouting` pointers still aliased the base model's — and the merge then wrote through them in place. Any built-in model carrying routing defaults would have had them mutated process-wide by one model's override, in an order-dependent way. Both merges now clone before writing, with a regression test (`TestMergeCompat_RoutingOverrideDoesNotMutateBase`). Latent today because no built-in model ships routing defaults; a live bug the moment one does.

### Documentation
- **OpenRouter / Vercel AI Gateway provider routing is now documented.** fir has emitted `provider: {only, order}` on OpenRouter requests (and `providerOptions.gateway` on Vercel AI Gateway) since the routing config landed, but the feature appeared in no `.md` in the repo and had no test — an aggregator pins a request to whichever backend it likes, and those backends differ in vision support, reasoning-effort handling, quantization and context window, so an undiscoverable pinning knob is the same as no knob. The `self` skill gains a "Provider routing" section: the key is the **per-model** `compat` block (there is no provider-level `compat`), reachable on a built-in via `modelOverrides`; routing is emitted only when the model's `baseUrl` matches the aggregator's host, so a stray key is inert rather than an error; and it names the `/endpoints` API for discovering valid provider slugs. It documents the full routing surface (see Added above). `TestModelRegistry_OpenRouterRoutingOverride` locks the JSON tag names, which are the user-facing contract.

### Changed
- **Model catalog regenerated**: added `amazon-bedrock/google.gemma-4-26b-a4b`, `amazon-bedrock/google.gemma-4-31b`, `amazon-bedrock/google.gemma-4-e2b`, `amazon-bedrock/in.openai.gpt-5.6-luna`, `amazon-bedrock/in.openai.gpt-5.6-terra`, `amazon-bedrock/us-gov.openai.gpt-oss-120b-1:0`, `amazon-bedrock/us-gov.openai.gpt-oss-20b-1:0`, `huggingface/google/gemma-3-12b-it` +2 more (+14 on aggregators); removed `amazon-bedrock/deepseek.r1-v1:0`, `amazon-bedrock/google.gemma-3-27b-it`, `amazon-bedrock/google.gemma-3-4b-it`, `amazon-bedrock/us.deepseek.r1-v1:0` (+2 on aggregators); 49 model(s) with changed pricing/limits.
...
- **One cache location and one collection policy for everything fir extracts out of its own binary.** Builtin skills, builtin extensions and the extension SDKs each answered "where does an extracted tree live, and who deletes it" differently: two hardcoded `~/.cache` paths, one `$TMPDIR` path, and no collection anywhere. New `pkg/cache` is the single answer.
  - Builtin extensions move from `$TMPDIR/fir-builtin-extensions/<hash>/` to `<cache>/fir/builtin-extensions/<hash>/`. This is also a bug fix: the `.complete` sentinel exists precisely because macOS periodic temp cleanup can delete an extension tree out from under a live session that is still launching subprocesses from it, and a cache dir is not swept by the OS. The legacy `$TMPDIR` location is collected, and its now-empty parent removed.
  - The extension SDKs (`<cache>/fir/sdks/<hash>/`) are collected for the first time. Every release shipped a new hash and nothing ever deleted the old one, so the directory grew one full SDK copy per version, forever — 22 of them on the host this was found on. This closes item 18 of `docs/pi-mono-compat-remaining.md`, which had it logged as known debt.
  - Collection is uniform: 14 days for cache trees, 3 days for the legacy `$TMPDIR` ones, aged out rather than deleted eagerly, because a running session started by an older binary still holds absolute paths into a tree the current binary will never choose. Each process refreshes the mtime of the tree it claims, so "old" means "unclaimed by any recent process", not "extracted long ago".
...
- **Asset-name parity test against the real distribution repo.** `TestDistkitAssetNameParity` fetches the latest `kfet/fir-dist` release and asserts distkit renders exactly the asset names it publishes, for all five shipped platforms plus `checksums.txt`. It skips when GitHub is unreachable or the anonymous API budget (60/hour, shared per IP by a NAT'd fleet) is spent; `FIR_PARITY_STRICT=1` turns those skips into failures for a release check.
...
### Added
- **`fir auth refresh [provider]` keeps an OAuth login alive with zero inference.** fir previously only refreshed an access token as a side effect of running a turn, so a machine that sits idle — a chat-relay bot host, a rarely-touched fleet box — eventually let its *refresh* token expire and needed an interactive browser re-login, which is miserable on a headless box. The workaround was a daily cron running a full agent turn (`fir -p "reply with: ok"`) purely to trigger the internal refresh path, burning subscription quota and starting a tools-enabled agent to achieve a side effect. `fir auth refresh` does exactly the refresh grant and nothing else. It refreshes **every stored account slot** of the provider (default account first), reports one terse tab-separated line per slot (`<slot> <label> <outcome> expires=<RFC3339> in=<duration>`), keeps going when a slot fails, and exits non-zero if any did — so cron and the journal can see it. It is generic over fir's declared OAuth providers: the token endpoint, client id, body encoding and headers all come from the extension's `declare_oauth_provider` spec and the `auth_post_exchange` hook still runs, so provider-specific normalisation (Anthropic's 5-minute expiry buffer) still applies. Nothing about Anthropic is hardcoded beyond it being the default provider name. By default a credential more than an hour from expiry is left alone rather than needlessly rotated (`--within DURATION` to change the guard, `--force` to override). Rotation and write-back go through the same flock-protected `auth.json` path fir's own mid-turn refresh uses — there is no second writer. Note for anyone testing it: copying the agent dir does **not** isolate a `--force` run — the refresh token in the copy is the same credential upstream, so the grant rotates the real one; a real `--force` test needs a separate login.

### Fixed
- **A live session no longer wedges permanently on `401 OAuth access token has been revoked` when its credential is rotated by another process.** Anthropic **revokes the previous access token the instant a refresh grant rotates the credential** — the old token does not linger until its stated expiry, it dies immediately. fir cached the resolved credential in memory and only ever re-read `auth.json` on an *expiry* it could see locally, so a session that was live across a rotation (another fir session refreshing, or `fir auth refresh` from cron) kept presenting a token that was already dead while its cached `expires` still read hours into the future. Nothing in the expiry-driven path would ever fire, and the session 401'd forever against an `auth.json` that was perfectly healthy on disk. `AuthStorage` now notices out-of-process rotation two ways: key resolution stats `auth.json` and re-reads it when the file changed (cheap, and it covers every provider, not just the one with a retry path), and the request path resolves through a new `RefreshApiKey` that re-reads unconditionally — so the provider's existing 401/403 retry sees a *different* key and retries once instead of giving up. Deliberately, an unchanged key after the re-read is surfaced as an error rather than triggering a fresh grant: a genuinely dead credential must not produce a storm of failing refreshes. This is what makes a cron keepalive safe to run on a host with live fir sessions.
...
### Fixed
- **`fir skills list|show` and `fir /<skill>` honour the `skills` paths in `.fir/settings.json`.** The CLI called `resources.LoadSkills` with `IncludeDefaults` only — the user dir, `.fir/skills` and builtins — never constructing the `ResourceLoader` a live session builds, so settings-declared skill directories and package-contributed skills were invisible to the inspection and slash-resolution commands while the running agent loaded them fine. All three call sites now go through the same loader (settings manager + package resolver), so the CLI and the TUI's `/skills` can no longer disagree.
...
- **`merge-to-main` skill: no check that local `main` is current**, which allowed a release to be cut on a base 10 commits behind `origin/main`. A new step 0 fetches and verifies before merging.
- **`merge-to-main` / `rebase-on-main` skills: CHANGELOG entries could land in an already-released section.** Rebasing across a release boundary relocates `[Unreleased]` entries conflict-free, and the existing diff check still looks clean. Both skills now require confirming entries sit under `## [Unreleased]`, not merely that they exist.
...
### Added
- **The `self` skill now enumerates the legal `api` values for custom models, and names the failure they cause.** `api` is the wire-protocol selector, so an OpenRouter fragment written with `"api": "openai"` (a provider name) loads fine, appears in `--list-models` and `/model`, and only dies per-request with `no API provider registered for api: openai` — a debugging cycle the skill's "Adding Custom Models" section did nothing to prevent. It now lists the eight `API` constants from `github.com/kfet/ai` with the command to re-derive them, states the model → provider → built-in-default precedence for `api` (and that only a built-in provider's model may omit it), and records that a *missing* `api` instead rejects the whole custom-model load — so every custom model vanishes, `--provider X` says `Unknown provider "X"`, and the real error is visible only in the interactive `/model` selector. Also documents `apiKey`/header resolution: `!cmd` runs a shell command, a bare value is used as an env var name when that variable is set, else literally — there is no `${VAR}` interpolation.
- **OpenRouter OAuth (PKCE) login is now a built-in auth extension** (`openrouter_auth.py`), replacing the config-dir drop-in. OpenRouter's flow is PKCE-shaped but not RFC 6749 — no `client_id`, the redirect travels as `callback_url`, no `state` round-trip, and the "token endpoint" (`POST /api/v1/auth/keys`) takes a JSON body and answers a bare `{"key": …}` — so it uses fir's imperative `auth_provider` path: the extension drives PKCE generation, the loopback callback server, the browser open and the exchange through the bridge helpers. The minted user-owned `sk-or-v1-…` key is stored in `auth.json` under the `openrouter` slot, where `AuthStorage.GetApiKey("openrouter")` finds it before any env-var or models.json fallback — so an OAuth'd OpenRouter account needs no literal API key anywhere in models.d/models.json. Keys never expire (no refresh token; `expires=0`, refresh is a no-op), and the provider record now carries `OAuthProviderID: "openrouter"` plus a KeyLink, so `fir login list`, `fir login openrouter` and `fir logout openrouter` treat it like every other first-class provider. Unit-tested end-to-end against a local fake exchange server (happy path, missing code, HTTP-error mapping, refresh no-op).
- **Migration note:** if you previously installed the `openrouter_auth.py` drop-in in `~/.config/fir/extensions/`, delete it — the built-in now owns the `openrouter` auth-provider ID, and the stale copy (which lacks `builtin: true` frontmatter) fails its handshake with "auth provider ID \"openrouter\" conflicts with built-in provider" on every `fir login`.

### Fixed
- **Two tests depended on the host's real environment variables and failed on machines with live credentials.** `TestRefreshLive_ClearsStateAndDiskCache` asserted that `RefreshLive` leaves `liveModels` empty, but `StartLiveModelFetch` re-populates it for any provider with auth — including an exported `OPENROUTER_API_KEY` on a developer's shell, which made the test fail with "expected liveModels cleared, got 1 entries" everywhere except CI. The test now clears the relevant key env vars via `t.Setenv`. Similarly `TestManager_StartFailures_Empty` scanned the host's global extensions dir (`~/.config/fir/extensions`) as part of discovery, so any user-installed extension that fails to start leaked into its expected-empty failure list; it now isolates `XDG_CONFIG_HOME` to a temp dir.
- **A user's `aside.json` advisor/delegate pin never took effect at all — the defect that made every other bug here so hard to diagnose.** `_advisor()` memoized on first access, and the `@fir_ext.tool` decorator evaluates it at *import* time — before the host's init handshake populates `fir_ext.config_dirs`. So the memoized value was always the built-in default, read from an empty config-dir list, and every later call returned that stale default. Because `aside` is a builtin, `ext_reload` refuses it, so nothing could dislodge it for the life of the session — and `/aside-advisor` confidently reported a value that was not the one in the file. The practical effect is worse than a config simply being ignored: there was no way to change the routing to test a hypothesis, and no way to observe what the routing actually was, so an investigation into a failing advisor could be sent down entirely the wrong path. The config is now re-read per access — two stat calls and a small JSON parse, free next to the LLM call it is about to route — so pins work, mid-session edits take effect on the next escalation, and `/aside-advisor` is a live view instead of an echo. (Whether the `escalate` parameter appears in the tool *schema* remains fixed at session start; the host collects schemas during the handshake.)
- **`aside escalate=true` hard-failed whenever the chain head failed without saying why.** The chain walk advanced only on `_is_model_unavailable_error`, which is substring matching (`404`, `not_found_error`, `does not exist`, …) against the provider's error text — but a provider can assert `stop_reason=error` and supply no text at all. Before `github.com/kfet/agent` v0.1.3 that case was laundered into `response had no usable content (blocks: []) (stop_reason=error)`, matching no signature, so the walk returned immediately with healthy candidates still sitting behind the dead head, breaking the module's own guarantee that "escalation NEVER disables itself on a dead chain". v0.1.3 (bumped here) intercepts it upstream and synthesises `provider reported stop_reason=error with no error message (provider=… model=… blocks: …)`, which fir now classifies as unavailability: cool the model off, advance the chain. It is deliberately *not* retried on the same candidate — upstream already classifies that text as terminal, so a retry would spend a round trip to reach the same verdict with less information — and it is matched as a short stable substring rather than a regex over the parenthesised tail, which is the part most likely to drift. A test pins the full verbatim v0.1.3 wording so upstream drift fails loudly instead of silently dead-ending a chain again.
- **Conversely, an empty response is now treated as the transient blip it is, including the `blocks: []` variant.** An earlier structural rule read an empty block list as proof of a failed call. That *was* true against agent < v0.1.3, where a message-less provider error wore exactly that costume. From v0.1.3 the costume is gone, so every remaining `no usable content` error comes from a stream that did not error — a live model that genuinely rendered nothing. It is retried once and advanced past, and the model is never cooled off. This also keeps the ambiguous residue safe: a provider that asserts neither an error nor a stop reason lands in this class and degrades gracefully rather than getting a healthy model benched. Note that fleet telemetry gathered before the bump counted `blocks: []` events that were in fact costumed hard failures; those now resurface under the synthesised wording, so the two populations are not directly comparable.
- **An advisor failure could still be a hard error even though a working executor model was sitting right there.** The terminal executor fallback ran only when the chain was walked to exhaustion. Now a route-shaped failure (transport, auth) stops the walk *without* cooling the model off — we have no evidence it is dead — and terminates on the executor, which answers with an `[advisor failed (<reason>) — answered on executor model]` note naming the actual fault instead of asserting an unavailability that was never established. Walking the remaining candidates was rejected: the default chain is all-Anthropic, so a provider-wide fault (429, bad key) would spend N doomed calls to reach the same executor. Two carve-outs surface immediately and are **not** retried or advanced, because they are attributable to the *request* rather than the route: context overflow (another model receives the identical oversized prompt, so the retry is near-certainly futile *and* expensive — it re-sends a whole context window of input tokens — and the existing "shrink the prompt" hint is the actionable answer), and user cancellation (firing another LLM call after Ctrl-C is actively hostile). The cancellation check runs first in the walk, so not even the empty-content retry can fire a call after an abort.
- **One blip banned the advisor for the entire session.** `_mark_model_unavailable` wrote a permanent tombstone into a module-level set, on the assumption that unavailability is a steady state — "the model is dead, stop probing it". The head of the default chain is *intermittent*: it fails some calls and answers others, so a single failure silently cost the best advisor until the process restarted, with no way to clear it (`aside` is a builtin, so `ext_reload` can't reset the module either). It is now a circuit breaker with backoff: a failure opens it for 60s, doubling per consecutive failure to a 30-minute cap, and a successful call closes it and resets the count. That settles "intermittent vs genuinely dead" empirically instead of by assumption — a blip costs one skipped escalation and recovers within a minute, while a genuinely dead model backs off to near-zero probe cost. Resetting the failure count on success (rather than only the deadline) is what stops an intermittent model ratcheting itself into the 30-minute cap over a long session.
- **Escalation could silently become a no-op.** When every candidate in the chain was already cooling off (or nothing was resolvable at all), chain resolution returned an empty list, no candidate was probed, and the executor answered with *no note* — because the note was gated on "we recorded at least one candidate error on this call". The caller had explicitly asked for advisor judgement, got its own model's answer back, and had no signal that the escalation never happened, so it would treat that answer as advisor-endorsed. Exactly the class of silent misdirection this whole change exists to remove. The note is now gated on "a role was requested and nothing advisory answered", covering both the chain dying on this call and the chain having been cooled off before it started; only a call that requested no role at all answers silently.
...
### Added
- **`handoff-nudger`, a new builtin extension: nothing was telling the agent when a session had become too expensive to continue.** Auto-compaction fires at 70% of the context window, which on a 1M-window model is ~700k tokens — so a long session happily re-reads a 400k-token prefix on *every* turn. Prompt caching works exactly as advertised (fresh `input` stays around 2 tokens/turn), and that is precisely the trap: the cost is entirely **cache-read**, it grows linearly with conversation length, and nothing bounds it. One measured 198-turn session ended at `cacheRead=422,917` on its final turn, 45.7M cache-read tokens in total, ~$46, and never compacted once — 4x the next biggest line item on that account, and the reason a sub window drained in under two hours. `self_handoff` (curated briefing plus bookmarks) already existed and is strictly better than compaction, which is lossy and in-band; the missing piece was the trigger. The extension reads `agent.info` after each `turn_end` and, once context crosses the lower of `atTokens` (150k) and `atPercent` (60% of window), injects a `[SYS_EXT]` note telling the agent to finish the current request, bookmark what matters and hand off — re-nudging every `nudgeEvery` (40k) further tokens, with `off` to disable. Configured via `handoff-nudger.json` in a config dir (project `.fir/` over `~/.config/fir/`), the same mechanism `aside` uses; no new core surface. Delivery is `ctx.prepend` and *only* `ctx.prepend`: `send_message` without `deliver_as` is log-only and the model never sees it, and `deliver_as="followUp"` calls `Agent.FollowUp`, which makes the agent answer the nudge out loud — in a relayed chat that is an unsolicited message to the user. `prepend_context` appends a `[SYS_EXT]`-wrapped user message to live agent state and deliberately does not touch the system prompt, so the nudge itself does not invalidate the prompt cache. A shrinking context (post-compaction, session rewind) resets the high-water mark, so the re-nudge interval cannot be stranded above a level it can never beat again. Verified end-to-end against a live agent: at 22k tokens silent, at 145k nudged, the agent finished its task, bookmarked the key result and handed off into a child session at 24.8k.
- **A coverage gate (`covgate`), adopted with an honest exclusion ledger rather than a green light that means nothing.** fir was the only repo in the family without one; it was also 67.0% covered, which is exactly why a straight `-min=100` gate would have been theatre — 212 of 237 files hold at least one uncovered statement, so a file-level ignore list that reaches 100% today has to exclude 95% of the tree, and 81% of what survives is `models_generated.go`, a generated lookup table that covers itself via `init()`. `make coverage` therefore runs **two tiers off one profile**. Tier 1 is the sibling convention verbatim — `-ignore=.covignore -min=100` — and it is an *invariant*, not a trend: one uncovered statement in a gated package fails the build. Its scope is small (372 statements) but it is real, because `pkg/ai` (hand-written), `pkg/envvars`, `pkg/mcp/history` and `pkg/modes/print` were taken to exactly 100% as part of this change rather than excluded. The `.covignore` patterns are deliberately **non-recursive** (`[^/]+\.go:`), which buys the two properties that make a 1.2% gate worth having anyway: a subpackage can be promoted without its parent (`pkg/session/store` need not wait for `pkg/session`), and a **brand-new package is gated at 100%** until someone consciously adds it to the ledger — so the gate's teeth are on new code. Tier 2 is the whole tree with no ignore file at `COVERAGE_FLOOR` (66), which catches catastrophic rot in the part tier 1 cannot see; ratchet it up, never down. The ledger itself is split into two labelled sections and the distinction is load-bearing: **structural** (process entrypoints, Bubble Tea rendering, self-update, re-exec, OS clipboard, generated code — covering these tests the terminal or the OS, not fir) versus **pure debt** (ordinary testable Go, 59.3% of the tree, listed with no excuse offered). Laundering the second as the first is how these files become permanent. `BACKLOG.md` carries the quantified debt and a promotion queue ordered cheapest-first — the first six units are sub-day, which is what stops a ledger of 2,000-statement packages from never moving. Gate lives inside `make all`: `-j` hides the ~38s non-race coverage pass under the race pass, and an unenforced gate is decoration. covgate is pinned as a **go.mod tool directive** (`go tool covgate`, matching acp-kit and slack-acp) rather than `go run …@version` — the version is then visible to `go get -u` and dependabot instead of rotting in a Makefile string no update tool reads, and it costs one `// indirect` require plus two `go.sum` lines because covgate has zero dependencies. `go-licenses` deliberately stays on `go run …@version`: a tool directive for it would drag ~25 requires and ~750 `go.sum` lines into the very module graph it exists to audit. `make test-cover` stays the exploratory per-function dump; `make open_coverage` browses the gated profile.
- **`pkg/ai`, `pkg/mcp/history` and `pkg/modes/print` are now at 100% statement coverage.** New tests cover the Bedrock IAM credential envelope (round-trip, bearer-token and malformed-JSON rejection), model/provider registry deregistration and priority ordering, the package-level `IsBuiltInApi` wrapper, the thinking-off fallback's pass-through and its post-start-error guard (a late error must *not* silently retry and duplicate streamed content), the alpha-prefix token split (`gemini.5` → `gemini` + `5`), malformed version tokens, unknown channel roles, and print mode's image attachment, prompt-failure and non-assistant-final-message paths. One genuinely unreachable branch was deleted instead of excluded: `ExtractFamily`'s empty-family fallback could not fire, because `modelIDTokens` always returns at least one token — `strings.Split` never yields an empty slice and every loop iteration appends. Its now-unused second return value went with it, and the invariant that licensed the deletion is now pinned by a property test running over adversarial ids *plus every model id in the shipped catalogue*, so it fails loudly if a future edit reintroduces a no-append path. Writing that test immediately earned its keep by catching a false assertion in its own first draft: degenerate ids that normalise away to nothing (`-`, `.`, `-preview`) legitimately yield an empty family, verified byte-for-byte identical before and after the deletion.

### Fixed
- **A test rollback silently deleted every built-in Bedrock model for the rest of the `pkg/models` run.** `TestBedrockAccountModelOverrides` registers one scratch model under the *real* `amazon-bedrock` provider, but rolled back with `ai.UnregisterProviderModels("amazon-bedrock")`, which deletes the whole provider key — taking all 118 genuine Bedrock models with it. `ai.GetProviders()` derives from those keys, so from that point on `amazon-bedrock` was no longer a *built-in* provider, and catalog-overlay validation (which may only add models to built-in providers) rejected the embedded catalog with a thoroughly misleading `"baseUrl" is required when defining custom models`. The damage stayed invisible until an overlay actually carried Bedrock entries, and then failed only in a full-package run — every affected test passed when run alone, which is the worst possible signature. The rollback is now the surgical `ai.UnregisterModel`, and a cleanup registered *before* it (cleanups run LIFO, so it runs last) asserts the provider still has models, pinning the invariant at the site that broke it.
- **Node SDK test failures were silently ignored by `make all`.** The `test-node-sdk` recipe was `command -v node && node …_test.js || echo "node not found — skipping"`, so a *failing* test took the `||` branch, printed the skip message and reported success — the entire Node SDK suite has been non-blocking. Found by deliberately breaking a Node SDK behaviour and watching the target still pass. Now an `if`/`else`: absent node still skips, a failing test fails the build.
- **Extension shutdown handlers were cut off mid-work, and `fir observe` reported cleanly-exited sessions as crashed.** Two independent bugs on the same path. First, the shutdown events (`session_end`, `session_shutdown`) were emitted fire-and-forget and followed by a blind sleep — 250ms in `Stop`, 500ms in `ShutdownAndCollect`, 100ms in `ReloadOne` — before the bridge context was cancelled and the extension processes killed. Any handler that outlived its grace lost its work permanently, with no error anywhere: on slow hardware (an RPi, a loaded CI runner) a handler doing real work simply never finished. Events are now delivered as JSON-RPC **requests** and awaited: the ack means the handler returned, so anything it did — a `set_session_data` call, a sidecar write — has already landed, and `ShutdownAndCollect` collects data that is complete by construction rather than by grace period. The wait is a *hard* cap (`shutdownAckTimeout`, 2s), deliberately not `CallHook`'s activity-aware deadline, which an extension with a chatty background thread would reset indefinitely and hang shutdown. Extensions are awaited concurrently, so the cost does not scale with extension count, and the happy path is now *faster* than the sleep it replaced — an ack arrives in milliseconds where every exit previously paid the full grace. All three bundled SDKs (Python, Node, Go) reply once the handler returns, even if it raised, and reply immediately when no handler is registered; an extension built against an older SDK never replies and simply waits out the cap, which is no worse than before. Second — and the actual cause of the observed misreporting — the observe extension's status was clobbered by event ordering: every event runs in its own SDK worker thread, so a slow `agent_end` (`status="idle"`) could land *after* `session_shutdown` (`status="ended"`) and resurrect a session that had already exited. `observe` classifies "pid gone but status still running/idle" as **crashed**, so a clean exit was reported as a crash. A longer grace period made this *more* likely rather than less, which is why no amount of waiting ever fixed it; `ended` and `crashed` are now terminal and no later event can overwrite them (non-status fields still apply, so late activity counters remain useful). The e2e regression test that caught this failed roughly one run in three and now passes 20 for 20.

### Changed
- **Model catalog regenerated** at release time: new upstream models picked up (`grok-4.20` multi-agent/reasoning variants plus `grok-4.3`/`4.5`/`4.6`, the `openai/*-fast` served-endpoint family, `Qwen3-VL-235B-A22B` instruct/thinking, `step-3.5`/`3.7-flash`, `tencent/hy3`), plus routine pricing/limit refresh.
- **Catalog overlay: the three Bedrock `global.openai.gpt-5.6-*` models now reach the fleet without a binary release.** v0.98.3 compiled them into `models_generated.go`, but the published overlay carried no `amazon-bedrock` entries at all, so every host still on an older binary was blind to them. Added as overlay entries carrying only what upstream asserts (id, name, api, pricing, context, modalities — no behavioural fields inherited from a sibling), which `publish-catalog.yml` mirrors to `kfet/fir-dist` on merge, so the fleet picks them up within the 1h TTL. `generatedAt` is bumped deliberately rather than left alone: a binary loads whichever of its embedded snapshot and its cached document has the later timestamp, so republishing with an unchanged timestamp would be silently ignored by every 0.98.3 host, whose embedded snapshot already carries it.
...
### Fixed
- **`fir` package updates died with "fatal: Cannot fast-forward to multiple branches" on 5 of 7 fleet hosts.** `pkg/pkg` `Pull()` ran `git pull --ff-only`, and `git pull` is the one git command that decides *what* to merge by reading `FETCH_HEAD` — a file git rewrites non-atomically. fir's own `Fetch()` (`git fetch --tags origin`) runs on these very clones at session start, so a second fir session on the same host interleaves its fetch with the fetch inside our pull, and the merge step then reads a clobbered file: duplicated merge candidates give "Cannot fast-forward to multiple branches", a lost entry gives "no such ref was fetched". This is a race, not per-host config — which is why it looked transient and why the dev box mostly escaped it. Measured before fixing: 200 pulls against a background fetch loop produced 63 failures with the exact field error. Notably a *statically* poisoned `FETCH_HEAD` does **not** reproduce it, because pull re-fetches and overwrites the file first; only a genuine race, or a `FETCH_HEAD` with several merge candidates at merge time, does. `Pull()` now resolves the tracked branch from the clone itself, fetches that branch **by name**, and fast-forwards to the remote-tracking ref — never consulting `FETCH_HEAD` at all. Ref updates are atomic, so a racing fetch can at worst leave the clone one commit behind, which the next update picks up. The upstream comes from `@{u}` with an `origin/<branch>` fallback rather than a hardcoded `main`, so a package pinned to a non-default branch still updates. A fetch that loses the ref lock to a racing fetch (`cannot lock ref …: is at X but expected Y`) is no longer fatal on its own: the rival fetch has already moved the ref, so the fast-forward is still attempted and the fetch error surfaces only if that fails too. **Detached HEAD** — a package pinned to a tag or sha via `CloneRef` — has no branch to fast-forward: `Pull()` refreshes refs and tags and returns nil, leaving the pin intact, because updating a pinned package is a re-pin through `CheckoutRef`, not a pull; previously it failed outright with "You are not currently on a branch". Same shape as the fix already shipped in `kfet/fir-exts` `7fd8a55` (`fleet/converge.sh`).
- **A transient empty response from the advisor killed the whole `aside` call — every third escalation, measured.** Anthropic intermittently returns a *successful* response (`stop_reason=stop`) carrying only a redacted thinking block and no text, which fir's side-query path surfaces as `response had no usable content (blocks: [thinking(th=0,sig=940)])`, plus a `blocks: []` variant. On 2026-08-11 that was 42 of 74 aside calls on one host (34 thinking-only, 8 no-blocks) and 15 of 56 on another — a steady drip since 2026-07-03, no burst, no hour-of-day pattern, no correlation with local config (neither host has an `aside.json`, their `settings.json` differ). It is transient: the same question that failed three times running succeeded on a re-probe hours later. Yet empty content was the **only** failure class in the advisor chain walk that dead-ended: a 404 gracefully walks `fable-5 → opus-4-8 → opus-4-7 → executor`, while an empty response got no retry, no chain advance and no executor fallback. It is now its own retryable class — retried **once** on the same candidate after a short backoff, then advancing to the next chain candidate, then falling through to the existing executor-model fallback, with the chained-error message preserved when everything is exhausted. Crucially the model is **not** memoized via `_mark_model_unavailable`: it is alive, it blipped, and memoizing would degrade the chain for the entire session over a hiccup. The retry and the advance are visible on the observable card (`retry:empty` / `advance:empty`) rather than silently swallowed, and the executor-fallback note distinguishes "returned no usable content" from "unavailable". Overflow and connection errors are deliberately **not** retried. Note the durable fix belongs in fir's Go SideQuery streaming client — one transport-level retry there would fix this for every `side_query` caller and make the same-candidate retry here redundant; this change is the graceful-degradation layer (chain-advance and executor fallback are policy only the extension knows), not the root fix.
- **The default advisor chain was hardcoded and had already drifted below the executor's own model.** `aside.py` shipped a static `fable-5 → opus-4-8 → opus-4-7` default while hosts were running `claude-opus-5` as their executor — so "escalate to a stronger model" could silently route to a **weaker** one, policed only by a unit test that forced a human to bump a constant on every registry addition. The default chain is now resolved at call time from `ctx.available_models()` (the session registry's live-and-authed set — a local bridge call, no network), ranked with the same helpers the degrade path already used: Fable tier above Opus, then major/minor, strongest first, top 3; delegate takes the highest live Haiku. New flagships are picked up with zero code changes. Two ranking bugs that only a live host exposed were fixed in the process, and both are exactly the failure this feature exists to prevent. The id patterns *required* a minor version, so the bare, live `claude-opus-5` was unrankable and the first dynamic chain came out byte-identical to the stale static one (`fable-5 → opus-4-8 → opus-4-7`) — the minor is now optional for every tier, and the chain on that host is `fable-5 → opus-5 → opus-4-8`. And date-stamped ids were rejected outright, but the only live Haiku on that host was `claude-haiku-4-5-20251001`, so the delegate role resolved to nothing and delegation was silently disabled — snapshots are now ranked strictly *below* the bare alias of the same version and deduped away when both are live, so the same model is never probed twice, while the snapshot is used verbatim when it is the only live spelling (never normalised to a bare alias the API may reject). Sonnet is still not considered (escalating to a mid tier defeats the purpose). Resolution never raises: an empty, erroring or Anthropic-less registry — or a host too old to implement the verb — degrades to the bundled static chain, which is now documented as a last-resort fallback only. Precedence is unchanged: an explicit `aside.json` `advisor`/`delegate` value still wins over everything, and `off`/`none` still disables the role. The two drift tests (`DefaultAdvisorTracksHighestAnthropicFlagship`, `DefaultDelegateTracksHighestAnthropicHaiku`) were **deleted** — pinning a fallback constant to the registry head is meaningless once the head is resolved dynamically, and a test that forces a manual bump for no behavioural gain is a test that lies. They are replaced by `DefaultAdvisorFallbackIsSane` / `DefaultDelegateFallbackIsSane`, which assert what still matters: the fallback specs parse, rank, are ordered strongest-first, and actually exist in fir's bundled model registry.
...
### Fixed
- **`rexec` (and everything built on it) failed against every Mac in the fleet.** The remote argv was `ssh <host> timeout -k 5 <secs> bash -l -s`, and macOS has no `timeout` — it is GNU coreutils, which Homebrew installs as `gtimeout` — so every single call to a Mac died instantly with `command not found`, exit 127, no work done; a whole incident had to be driven through raw `ssh` in Bash instead. The bound is now a **one-line bash supervisor passed as the ssh command**, needing nothing on the remote host but bash and `sleep`: it backgrounds `bash -l -s` under job control, so the command leads its own process **group**, and a watchdog TERMs that group, waits the grace, then KILLs it, reporting the same clean 124 with partial stdout/stderr. The trick that keeps it this small is that `set -m` does two jobs at once — POSIX only redirects an async command's stdin to `/dev/null` when job control is *off*, so the backgrounded child still reads the user's script **verbatim off the ssh channel**. Nothing is staged, encoded or re-quoted, and `$0`, `$$` and shell error prefixes are exactly what they were before the bound existed. No probe (an extra round trip per host that still fails where neither binary exists) and no per-host shim to install. Two details are load-bearing and easy to get wrong: the supervisor text carries no newline and no single quote, so one `shlex.quote` makes it survive being re-parsed by whatever login shell the host has (zsh on macOS, even csh), and `ssh -T` is now mandatory — a background job reading from a *tty* takes SIGTTIN, so against a host with `RequestTTY yes` the call would return 149 having run nothing. A TERM/HUP/INT delivered to the supervisor is forwarded to the group; when the *local* side gives up first the remote is usually never signalled at all (no pty means no HUP, and the ssh mux holds the channel open), so the watchdog is the backstop there, bounded by `timeout_s` — GNU `timeout` could not see a dead client either. The old "remote host lacks GNU `timeout`" hint is gone: we no longer run it, so a 127 about `timeout` is now honestly the user's own command failing.
- **`rexec detach=True` silently never ran the job on macOS.** The non-systemd launcher was `( setsid /bin/bash runner & ) &`, and macOS has no `setsid` either — the subshell failed, the outer fork still exited 0, `rexec` reported success, and `rjob` then sat in `state=unknown` with an empty log forever. There is now a third tier, `( set -m; nohup /bin/bash runner & ) &`: job control gives the runner its own process group (so `rjob kill`'s `kill -TERM -$PID` still signals the group, since the runner records its own `$$`) and `nohup` shields it from the HUP when the ssh session goes away.
- **A `rexec` timeout that produced no output at all now says why it might have.** The remote bound's clock starts when the supervisor launches `bash -l`, so it covers the child's *login-shell startup* — sourcing `/etc/profile`, `/etc/profile.d/*`, `~/.profile` — exactly as the old `timeout -k` argv wrapper did. Against a host with a fat profile a very small `timeout_s` can therefore expire before the user's first line ever runs, and the envelope that comes back is a bare `outcome=timeout`, exit 124, both streams empty: indistinguishable from a command that hung, one that prints nothing by design, and one that never started. A 124 with nothing on either stream now carries a hint naming the login-shell-startup case and pointing at `timeout_s`. It asserts only what is true, so it stays honest for a legitimately silent `sleep 999`; a timeout that *did* produce partial output is left alone, because the output is the diagnosis and a hint would only add noise. This surfaced as a red `test_expiry_kills_the_whole_process_group_and_reports_124` on CI only: the test hardcoded a 1-second bound, which on a loaded GitHub runner expired while the child was still sourcing profile, so stdout was legitimately empty rather than lost (data already written into a pipe cannot be destroyed by killing the writer — the kernel holds it until the reader drains it). The test now measures the login-shell startup the machine actually pays and scales its bound to it, and a new sibling pins the opposite contract — a bound expiring *during* startup reports a clean 124 with empty streams — which is load-monotonic by construction: heavier load makes the profile slower, which makes the assertion more certain rather than flakier. Verified on two real Linux hosts including a 1-vCPU box under 8x CPU oversubscription.
- **The v0.98.1 "remote host lacks GNU `timeout`" hint is superseded and removed.** v0.98.1 shipped a shell-agnostic fix for that hint (it only matched bash's wording, so the zsh phrasing macOS actually emits — `zsh:1: command not found: timeout` — never fired it). The wrapper removal above lands in the same release and deletes the argv-level `timeout -k` the hint existed to explain, so the code path can no longer execute: `_missing_remote_timeout` and `_RC_NOT_FOUND` are gone with it. A 127 mentioning `timeout` from `rexec` is now honestly the user's own command failing, which is what the hint was there to deny.
...
### Added
- **Nightly model-watch: fir now notices new models by itself, and opens a PR instead of waiting to be told.** The catalog overlay (v0.86.0) solved *distribution* — merging `pkg/models/catalog-v1.json` updates the whole fleet without a release — but nothing solved *detection*: a human still had to read a launch blog and go looking, which is how Claude Opus 5 cost an evening. A scheduled `model-watch` workflow now runs `cmd/generate-models` against upstream every night, diffs the result against the catalog compiled into the checkout, and opens (or updates) a single rolling PR on `bot/model-watch` when upstream has models fir does not know about. It never merges, never releases and never publishes the overlay — publishing stays a push-to-main trigger, so the human merge is the approval gate. The PR body distinguishes genuinely NEW models from mere metadata churn and, for each new model, names the fir-specific curated fields (compaction, adaptive thinking, reasoning-effort values, SWE score, compat) that its closest compiled-in relative has and it does not — the relative being found with the generator's own lineage heuristics, now shared in `pkg/ai/family.go` (`ExtractFamily`, plus a generation-stripped `ExtractLineage` and a comparable `GenerationVector`) rather than a second set of guesses. Noise, not detection, is the failure mode that kills a feature like this, so the default `-trigger curated` opens a PR only for a new model on a first-party provider, or one whose product line fir already ships; pure pricing/limit churn (219 models on a typical night) and models that merely vanished upstream are reported but never trigger, aggregator suffix variants (`:batch`, `:free`, `:exacto` — OpenRouter enabled `:batch` for ~55 ids in one go) are recognised as variants of models fir already has, and models already shipped by the committed overlay count as known. Past 50 qualifying models the run declares an upstream anomaly and opens nothing. `-trigger all-new` / `any-diff` and an overlay toggle are available from `workflow_dispatch`. Upstream flakiness produces neither a PR nor a red badge: `-strict` treats any failed source as a no-run, because a partial fetch looks exactly like a mass deletion. When a PR is opened the run also proposes the corresponding catalog-overlay entries, so merging reaches the fleet within the TTL without a release; those entries carry only what upstream asserts (id, name, api, pricing, context, modalities, and this model's own compat) — never a behavioural field inherited from a sibling, never an inferred SWE score, and never `baseUrl`/`headers`, which the overlay validator forbids. Hand-curated overlay entries are never rewritten, a model whose compat the overlay schema cannot express is skipped with a reason instead of published in a quietly weaker form, and the proposed document is validated with `models.ParseCatalogOverlay` — the very code that loads it in production.
- **`/doctor` warns when a stale `defaultModel` pin is shadowing a newer provider default.** Three layers can set a default model (app/bot config → per-host `settings.json` → the provider's own `DefaultModelID`, which the catalog overlay can move without a release) and a stale lower layer silently beat a newer upper one forever — a host sat on `claude-opus-4-6` long after the built-in default moved to `claude-opus-5` and nothing said a word. `session.Introspection` (the `agent.info` bridge method) now carries a `diagnostics` array of actionable configuration problems, and the builtin `doctor` extension renders it above the failure history, naming the pinned value, the value it shadows, the exact file to edit, and the two ways out. It is a warning only — an explicit pin is a deliberate act and is never rewritten. The bar for warning is deliberately high, because a wrong warning trains the operator to ignore the check: fir stays silent unless the provider's own current default is a strictly newer generation of the *same product line* as the pin, with both ids known to fir and both carrying a comparable generation. No pin, a current pin, an id fir does not know (very likely a valid custom `models.json` model), a different line (`opus`→`sonnet` is a cost choice, not staleness), a dated or `-preview` spelling of the same generation, a pin *newer* than the default, and any id pair that cannot be ordered (`kimi-k2-thinking`, `deepseek-r1`) all stay silent.

### Fixed
- **`make test` failed on any machine with `tag.gpgsign = true`.** The `pkg/pkg` git fixtures set a fixed author identity by environment but inherited the developer's global git config, so a perfectly normal signing setup turned `git tag v1` into a signed annotated tag and six tests died with `fatal: no tag message?` — on that machine only. Both fixtures now go through one `testGitEnv()` helper that also neutralises `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM`.

## [0.97.0] - 2026-08-09

### Fixed
- **`forge_tool` could write an extension that fir would never load, and report it as success.** Forging an extension whose source lacked comment frontmatter produced a syntactically valid file in `~/.config/fir/extensions/` that `scanExtDir` skipped forever and silently (a file is an extension only when it is executable **and** carries a `# ---` frontmatter block **and** is not `builtin: true`) — while forge replied "loaded extension '<name>' … No new tools detected", which reads as success and costs an hour of debugging. `forge_tool` now validates and repairs the source *before* writing: it compiles it (syntax errors are rejected up front, with the line number), prepends `#!/usr/bin/env python3` when missing, and injects a minimal `# ---` frontmatter block with the extension's `name` when absent. Frontmatter that would defeat discovery or mislead is a hard error instead of a silent write: a malformed `# ---` block, `builtin: true`, or a `name:` that disagrees with the file basename; a valid block merely missing `name:` is repaired in place. After `reload_extension`, forge no longer guesses from a tool-list delta — it asks the host, over a new `list_extensions` RPC (`ctx.list_extensions()` in the Python SDK), which extensions are *actually running*, and returns `is_error` naming the file and likely causes when the freshly written one is absent. An extension that loads but registers no tools is now reported as loaded-with-no-tools, so the two cases are distinguishable; an extension mode-gated out of the current session is reported as written-but-not-loaded rather than as a failure; and a host that cannot answer (no extension manager wired) gets wording that asserts neither. Injected frontmatter deliberately declares no `modes:`, so a forged extension loads in every mode. Separately, `scanExtDir` stopped being silent: skipping an executable file for missing frontmatter logs at debug with the path and the fix, and a file that *has* valid non-builtin frontmatter but lacks the executable bit logs at warn. Expected skips (`builtin: true`, files that are not extensions at all) stay quiet.
...
### Added
- **`autoresearch`: benchmark integrity lock + wall-time efficiency stats.** An autoresearch campaign optimises the metric emitted by `autoresearch_bench.sh`, but nothing stopped an experiment worktree from editing that script — a change could rewrite its own benchmark, "win", and get merged with the tampered script, invalidating the whole campaign (a self-confirming loop). A new `lock_benchmark` tool computes the sha256 of `autoresearch_bench.sh` and writes `autoresearch.lock` (sha256, path, iso timestamp, git short sha) in the campaign root, next to `autoresearch.jsonl`; the `autoresearch-create` skill calls it right after the baseline. `run_experiment` now finds that lock by collecting every `autoresearch.lock` across the repo's worktrees and deduplicating them **by content** (the campaign's lock is routinely committed, so sub-worktrees inherit identical copies — one logical lock); one distinct lock governs the run, and several distinct locks are refused as ambiguous rather than guessed at, so an experiment cannot authorise itself by re-locking its own rewritten benchmark. It hashes the benchmark before running and **refuses** (returns `success: false`, `is_error`) when it differs, naming both hashes and the fix (revert the change, or re-baseline + re-lock with user consent). Deleting the benchmark, or a lock that is unreadable or missing its hash, is refused too. Fully back-compat: with no lock file present it runs exactly as before, noting that no lock is set. Separately, `run_experiment` measures and returns `wall_ms`, `log_experiment` persists it, and `/autoresearch` now reports total wall time, keep rate (keeps/total), and wall time per kept win. No token-cost numbers are invented — the extension cannot see them.

### Fixed
- **Installed fir packages' extensions and skills were invisible under `--mode acp`.** A package installed with `fir install` (recorded in `settings.json`, its files under `~/.config/fir/packages/…`) loaded normally in the TUI — `started extension ext=reminders` — but under `--mode acp`, with the same cwd and only `--mode` differing, its extension was never discovered and its `SKILL.md` was absent from the skills list; copying the same file into `~/.config/fir/extensions/` made it load, proving it was neither the mode filter nor the file. The cause was that the ACP session-setup path never wired the resources the `cmd/fir` CLI/TUI path did: it built the `ResourceLoader` with **no `PackageResolver`** (so `GetPackageExtensionPaths()` was empty and package-contributed skills never loaded) and called `extension.Setup` with **neither** the package extension files nor the settings `extensionPaths` directories. A package-provided *auth* extension could not load at all, because ACP's `Initialize` auth-provider bootstrap omitted those extra sources too. ACP now wires the `PackageResolver` into its resource loader and feeds both the package extension files and the resolved settings dirs into the session extension setup and the auth bootstrap; the auth path gained an `ExtraExtensionFiles` source, which is load-bearing rather than redundant — the manager re-runs discovery on reload / lazy `session_start`, so without it package auth extensions would be dropped on the second pass. To close this drift class — where a new mode or a new source kind silently misses wiring the CLI already had — the two overlapping option fields collapse into one embedded `ExtraSources{Files, Dirs}` on both `SetupOptions` and `AuthSetupOptions`, populated by a single `extension.ResolveExtraSources` constructor shared across all four call sites (CLI, `login`, ACP session, ACP auth), and the thrice-repeated "append-if-absent by name, with project/global shadowing package" merge is extracted into one `mergeConfigsByName` helper that is now the sole home of that precedence rule. The `# modes` filter was untouched.
...
- **A tool call orphaned by process death is answered on resume instead of silently re-run.** When fir is killed mid tool call (e.g. the `fir --mode acp` child dropped on a relay worker swap) the transcript's last assistant message carries `toolCall` blocks that no `toolResult` ever answered. The wire-time backstop in `providers.TransformMessages` already kept providers from rejecting that history, but it filled the gap with `"No result provided"` — indistinguishable from a tool that genuinely failed, so the model's natural move was to just re-run the call and duplicate its side effects (file writes, `git push`, HTTP POSTs). Session load now synthesizes a distinct error result for every unanswered tool call, stating plainly that the process was terminated before the tool returned and that any side effects are UNKNOWN and must be verified before retrying. It lives in `store.BuildSessionContextFromEntries`, the single choke point every resume path goes through (`fir -c`, ACP `session/load`, `/resume`, tree navigation), handles several parallel tool calls in one message, repairs orphans mid-history as well as at the tail, leaves calls that already have a result anywhere in history untouched, and skips errored/aborted assistant messages (which the provider transform drops, so a result there would orphan itself). The synthesized result is derived on each load and never written back to the transcript. Separately, a session file whose final line was truncated by a SIGKILL mid-write is now repaired with a newline at open, so the next appended entry can no longer be glued onto the partial record.
- **`tmuxspinner`: our own repaint is no longer mistaken for a user rename.** The ticker thread snapshotted `_last_set` under the lock, then read the tmux window name *outside* it. A rename issued by another thread of ours — `start()`'s initial paint, which runs on the caller's thread after the ticker is already spawned, or `set_session_name` — could land in that gap, so the read returned a name we had set that no longer matched the stale snapshot. That was mis-detected as a user rename and the spinner/session suffix was baked into `_original_name` (`zsh` → `zsh mysess`), after which `shutdown()` "restored" the window to a name the user never chose. Detection now re-checks under the lock that `_last_set` is unchanged before treating a difference as external, so only genuine user renames are adopted. This surfaced as an intermittently red `test_shutdown_restores_original_name`, which failed the v0.92.1 release run.

## [0.92.0] - 2026-08-02

### Fixed
- **`wait` timeouts are now resumable checkpoints.** Hitting the `timeout`/`max_polls` cap used to discard everything — the probe `steps`, the poll counter, and the `$WAIT_STATE` scratch file the probe had accumulated settle/delta data in across dozens of polls — so "re-issue with a larger cap" meant re-authoring the chain and starting the state from scratch. A timeout now stashes that state under a short handle and reports it as `resume: w_8f3a21`; calling `wait` with `resume: <handle>` (steps optional) re-enters the same loop with the same scratch file and contents, the poll counter continuing where it stopped, and a fresh cap budget. Caps passed alongside `resume` override the originals; omitted ones are reused. Handles are session-scoped, expire after 2h, and are dropped — with their tempfile — on any non-timeout outcome, on an unknown/expired-handle error (which lists the valid handles rather than silently starting over), and at process exit, including files of loops still in flight.

## [0.91.0] - 2026-08-02

### Added
- **`remote` builtin extension — ssh execution and remote tmux driving as first-class tools.** Measured across real fir sessions, 59% of all `Bash` calls contained ssh/scp/rsync and each paid a per-call correctness tax: 2639 escaped-quote sequences, 881 `timeout N`, 557 `bash -lc "..."` wrappers, 555 `ConnectTimeout=`, 247 heredocs. `remote.py` converts that recurring tax into one implementation with six tools — `rexec` (script shipped on **ssh stdin** to `bash -l -s`, never `ssh host "bash -lc '...'"`, so nothing is re-split by an intermediate shell and login-shell/zsh-host semantics are guaranteed; `detach=True` runs it under `systemd-run --user --collect`, falling back to a double-forked `setsid`), `rjob` (status/log/tail/kill over remote-side state in `~/.cache/fir/rjobs/`, no local registry to drift), `rput`/`rget`, `rtmux` (`ls|new|send|cap|kill` — remote tmux **is** the PTY, every action a stateless short exec with no held `ssh -tt`; `cap` returns `unchanged: true` for a byte-identical pane so polling is cheap), and `rhosts` (reads `~/.ssh/config`, optional parallel reachability sweep). Connection flags including a `ControlMaster` mux are owned by the extension, and every tool returns a discriminated envelope (`ok | nonzero_exit | timeout | unreachable | auth_failed | no_tmux | no_target`) with `stdout_bytes` always explicit, so an empty success can never be misread as no signal — a remote command exiting nonzero is signal, not a tool error. Non-UTF-8 remote output is decoded with replacement rather than raising, and truncation slices on bytes, so a binary `cat` or a huge log still comes back as a well-formed envelope. Auth classification keys on the method list ssh always prints (`Permission denied (publickey)`), so an ordinary unwritable remote path — which `scp` also reports as "Permission denied", with the same exit 1 it uses for everything — stays `nonzero_exit` signal instead of being flagged as a transport failure.
...
### Fixed
- **`observe`: `host_pid` now points at the fir binary, not the python forkserver.** The sidecar recorded `os.getppid()`, which under the forkserver architecture is the *python forkserver* process — so `stop_session` signalled the wrong pid, the target session survived, and the session listing went stale. The fir host now exports `FIR_HOST_PID` (`pkg/extension/sdk/env.go`) and `observe.py` reads it, falling back to `getppid()` for older hosts or a missing SDK. Covered by a Go forkserver test plus python tests for both the env-var and fallback paths, including the negative assertion that the recorded pid is *not* `getppid()`.
- **`wt` skill: portable `mktemp` template in `spawn.sh`.** `mktemp -t firtask` relies on BSD semantics; GNU coreutils requires at least six trailing `X`s, so spawning a worktree agent failed on Linux. Now `firtask.XXXXXX`.

### Changed
- **Refreshed the generated model catalog** (`make generate-models`).
- **Bumped `kfet/agent` to v0.1.2**, which corrects the `bash` tool description. It claimed daemons detaching via `setsid` **or double-fork** escape the post-command cleanup; the double-fork half is false, because cleanup is a process-*group* kill and `fork()` preserves the process group. The description now states the underlying rule — cleanup kills the process group, `setsid` starts a new one, and it does not start a new cgroup — and leaves the consequences to be inferred.
- **The release workflow no longer hangs forever on a retired runner.** `brew-smoke` matrixed over `macos-13` to cover Intel macOS; GitHub has retired that image, so the label stopped resolving and the job sat in "waiting for a runner" until cancelled by hand — v0.89.0 for ~7h, v0.90.0 for 50m+. Artifacts were never at risk (the job is post-publish with `fail-fast: false`); only the run's terminal state was affected. The Intel leg now uses the successor label `macos-15-intel` (verified out-of-band: picked up in 15s, reports `Darwin x86_64`), and `timeout-minutes` was added. Note that `timeout-minutes` does **not** bound queue time — it only starts once a runner picks the job up, so it guards a wedged `brew install`, not an unresolvable label; GitHub's ~24h queue cull is the only backstop there. Intel coverage is worth keeping — fir ships `darwin-amd64` and the formula's `/usr/local` Intel path differs from arm64's `/opt/homebrew` — so a new manual-only `runner-probe.yml` validates a candidate runner label out-of-band. Use it before changing any `runs-on` label: an unverified label can then never hang a release run.

## [0.90.0] - 2026-07-27

### Added
- **Remote MCP servers now authenticate over OAuth, with zero configuration.** fir previously had no auth for HTTP MCP servers at all — `ServerConfig` carried no headers and no credentials, and both HTTP transports were built with a nil `HTTPClient`. A bare `{"url": …, "transport": "streamable"}` entry now Just Works against any spec-compliant server: fir connects unauthenticated and, on a `401`, runs the MCP authorization-spec discovery chain automatically — RFC 9728 protected-resource metadata (from the `WWW-Authenticate` `resource_metadata` parameter, falling back to well-known path derivation when the 401 carries no challenge, and to the resource origin when no document exists), RFC 8414 authorization-server metadata across every advertised issuer and all three well-known forms, and RFC 7591 dynamic client registration — then an authorization-code + PKCE (S256) login carrying the RFC 8707 `resource` indicator on the authorization, token **and** refresh requests. Tokens live in the existing `auth.json` under an `mcp:<server>` key (flock, 0600, atomic rewrite), survive a restart, and are **bound to the server URL they were minted for** so a project-local `.fir/mcp.json` cannot redirect a name at a hostile host and inherit its token. A rotated refresh token is persisted on every refresh. The bearer is attached only to requests whose origin matches the credential's, and the MCP data-path client refuses to follow redirects — net/http's own cross-host `Authorization` stripping runs *above* the transport, so without both locks a server answering `302` to a host of its choosing would be handed the token.
  - The transport half (`pkg/mcp/oauth.go`) is a small `http.RoundTripper` rather than the SDK's `auth.HTTPTransport`: the latter sits behind the `mcp_go_client_oauth` build tag and is documented one-shot ("subsequent 401s are not processed"), which cannot recover from a revoked token in a long-lived agent session. Ours attaches the bearer, refreshes 60s ahead of expiry, silently refreshes and replays once on a 401, and drops the credential when the refresh token is itself dead. It never blocks on a human — a genuine login requirement surfaces as `*mcp.AuthRequiredError`, which the dial path reports verbatim because its message names the command that fixes it.
  - **fir never opens a browser on its own — structurally.** The `Manager` holds no login-UI hook, so no reconnect cycle, background goroutine or non-interactive mode can start a browser flow; MCP servers connect from goroutines that may predate the TUI and must not touch its widget tree, and the reconnect loop re-dials forever. A server needing credentials reports `run: fir mcp login <server>` on every cycle and stops there. New `fir mcp list|login|logout <server>` subcommand and `/mcp login|logout <server>` slash commands are the only paths that open a browser (loopback callback raced against a manual paste prompt, so SSH works). `fir mcp login` probes the server unauthenticated first so it can use the 401 challenge's `resource_metadata` pointer rather than guessing the well-known path. While a login is in flight, other requests to that server fail fast with "an interactive login is in progress" instead of queuing behind a human, and the auth lock is acquired with the caller's context so a request never outlives its own deadline.
  - **Credentials never ride plaintext.** A `sse`/`streamable` server whose URL is not `https://` is rejected before any connection, unless the host is loopback (`127.0.0.0/8`, `::1`, `localhost`) so local dev servers keep working. This covers OAuth bearers and static PATs alike — `http://mcp.internal:8080/mcp` would otherwise put every token on the wire in the clear. `"auth": {"mode": "none"}` sends nothing and is therefore still allowed over plain HTTP.
  - Optional `auth` overrides, all absent by default: `mode` (`oauth` / `bearer` / `none`), `token` (static PAT, with `${VAR}` environment expansion), `client_id`/`client_secret` (authorization servers without dynamic registration), and `scopes`. Documented in `docs/mcp.md` and the `self` skill.
  - Tests use a strict fake authorization server plus a real OAuth-gated MCP server (`pkg/mcp/oauth_testserver_test.go`) that reject a missing PKCE verifier, a missing `resource` parameter, a mismatched `redirect_uri` and a revoked refresh token — covering discovery from header, discovery without a header, dynamic registration, pre-registered client, proactive refresh, silent refresh after 401, re-auth after full revoke, static bearer (literal and `${VAR}`), opt-out, persistence across restart, URL binding, plaintext refusal, redirect containment, fail-fast during an in-flight login, and the structural guarantee that the dial path never starts one.
...
### Changed
- Embedded skills no longer carry the author's personal paths or email. `extract-module` pointed at `/Users/kfet/dev/ai/` for its sibling repos — a path that does not exist on a Linux box, let alone on anyone else's machine — and now says `~/dev/ai/`; `telegram-bot-setup`'s `.fir/mcp.json` example had two hardcoded `/Users/kfet` paths, now placeholders plus a note that `~` is **not** expanded in that JSON so the value must be absolute (`command -v bun`); `claude-usage`'s sample output carried a real personal email address. These ship inside every binary, so they were wrong for every user but one.
- **`pkg/tui` is now the standalone module [github.com/kfet/tui](https://github.com/kfet/tui) v0.1.1**, adopted as a dependency; the in-tree copy is deleted (-7,234 lines). The terminal toolkit — ANSI parsing, grapheme- and width-aware measurement and wrapping, key/keybinding parsing (legacy + Kitty protocol), inline image rendering (Kitty/iTerm2) and the overlay-capable render loop — had no fir-specific coupling and is useful on its own. `pkg/tui/components` (editor, input, markdown, selectlist — the fir-shaped widgets) stays in-tree and now imports the external package. The extracted module carries a 100% coverage gate (1,632/1,632 statements) with the real-TTY adapter isolated in `terminal_process.go` and excluded via `.covignore`; unreachable defensive branches were pruned to reach it, verified behaviour-preserving against the in-tree copy by differential testing (4,000 randomized and truncated image inputs plus the fuzzy-filter cases produce identical results, and `ParseKey`/`MatchesKey` for `delete`/`pageUp`/`pageDown` are unchanged). Standing obligation, as with `kfet/ai` and `kfet/agent`: TUI improvements must now be released upstream before fir can adopt them.
...
### Fixed
- **`wait` no longer reports a timeout as an error, and defaults to 900s instead of 300s.** Measured across 517 local session transcripts, `wait` set `isError` on 64 of 125 unique calls (51.2%) — the worst rate of any tool — and 44 of those were plain cap-hits on legitimately long jobs (release workflows, fsck runs, tailnet waits), not failures. A probe loop that has run clean for 36 polls and simply has not settled yet is a *partial result*: flagging it poisons tool error-rate metrics and trains the model to read "still running" as "broken". The timeout message now names which cap bound the loop (`max_polls` vs wall-clock), shows both budgets, and says to re-issue with a larger one; the missing-sentinel error now quotes the offending final stdout line, since that error is an authoring bug and the line is what fixes it. Reclassifying drops the observed error rate to roughly 14%, in line with the rest of the toolset.
...
### Added
- User-level `mcp.d` drop-ins: MCP configs can now be placed in `~/.config/fir/mcp.d/*.json` (or `$FIR_AGENT_DIR/mcp.d/`). Files are loaded in lexical filename order and merged after the user-level `mcp.json`; later files override earlier ones when the same server name appears. Collision detection reports shadowed servers to the `reload_mcp` tool result so agents can see and fix conflicts.
- `reload_mcp` bridge method: extensions can call `ctx.reload_mcp()` to re-read MCP config from disk and apply changes mid-session. Returns collisions (when server names are shadowed across config files) and errors so the agent can fix and retry. This enables AI sessions to self-serve MCP servers by writing drop-in JSON files and reloading.
...
- AI/agent extraction Phase 5. The portable AI primitives (`pkg/ai/core` +
  `ratelimit`/`overflow`/`jsonparse`) moved to a new sibling module
  `github.com/kfet/ai`, and the agent runtime (`pkg/agent` +
  `pkg/agent/tools`) to `github.com/kfet/agent`. Both modules are published
  at `v0.0.1` and `v0.0.2` respectively; fir depends on them directly (no
  `replace` directives). The in-tree copies were deleted; `pkg/ai/aliases.go`
  re-exports from `github.com/kfet/ai` so the 150+ fir call sites are unchanged.
  Behaviour is byte-identical — a mechanical move. See
  `docs/design/ai-agent-extraction.md`.

- The Python extension forkserver (warm COW fork template) is now a process-lifetime singleton shared across all sessions, keyed by SDK hash, instead of one per session. Under `fir --mode acp` (one process, many sessions) this collapses N byte-identical warm interpreters to one; session end / `session/release` still stops and reaps that session's forked children but leaves the shared template running. The template now also removes its own `/tmp/firfork*` socket dir on exit (passed via argv), so no temp dir leaks even when fir never gets to clean up (normal exit, `/reexec`, crash).
...
- New `provider_error` extension event, emitted on `turn_end` when a turn ends
  in a provider/LLM error (assistant `stop_reason == error`). Payload carries
  `error_text`, a classified `kind` (`rate_limit`/`overloaded`/`server`/
  `transport`/`terminal`), `retryable` (via `pkg/ai/ratelimit`), `provider`,
  `model`, and `retry_after_ms` when the provider indicates one. Documented in
  `docs/extension-protocol.md`, mirrored in the Python SDK, and exercised by the
  demo extension.
- The `schedule` extension now auto-resumes the session after a *retryable*
  provider error (Anthropic Overloaded/529, rate limit/429, transient 5xx).
  It honours the provider's `retry_after` when given, otherwise backs off on a
...
- Dracula theme: brightened dim/muted text (status bar, tool output, markdown quotes/URLs, thinking text) for better contrast against the dark background
