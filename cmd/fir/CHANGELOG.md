...
### Fixed
- **`rexec` (and everything built on it) failed against every Mac in the fleet.** The remote argv was `ssh <host> timeout -k 5 <secs> bash -l -s`, and macOS has no `timeout` — it is GNU coreutils, which Homebrew installs as `gtimeout` — so every single call to a Mac died instantly with `command not found`, exit 127, no work done; a whole incident had to be driven through raw `ssh` in Bash instead. The remote bound is now a **self-contained bash wrapper shipped in the same stdin payload** rather than a binary that has to exist over there: the script is staged to a temp file through a collision-checked quoted heredoc, run as `bash -l <file>` in its own process group (`set -m`), and watched by a sleeper that touches a flag, TERMs the *group*, waits the grace period and then KILLs it — reproducing exactly what `timeout -k` gave us (bounded on the remote side, whole group signalled, clean 124 with partial stdout/stderr) with nothing but bash, `mktemp` and `sleep`. No probe (an extra round trip per host that still fails where neither binary exists) and no per-host shim to install. The remote argv shrinks to `bash -s`, so nothing depends on the login shell either — macOS defaults to zsh. Three details that were failures waiting to happen: children get `</dev/null` because the wrapper's own script is still arriving on stdin as it forks; the wrapper's stderr is swapped for `/dev/null` before `wait` (the command keeps the real one on fd 3) so job-control `Terminated: 15` notices never pollute the user's stderr; and a TERM/HUP/INT delivered to the wrapper — sshd tearing the session down — is forwarded to the group, which `timeout` did for us and a naive backgrounded job would not. (When the *local* side gives up first the remote is usually never signalled at all — no pty means no HUP, and the ssh mux holds the channel open — so the watchdog is the backstop there, bounded by `timeout_s`; GNU `timeout` could not see a dead client either.) A stage that lands short of its exact byte count refuses to run rather than execute a script truncated by ENOSPC. The old "remote host lacks GNU `timeout`" hint is gone: we no longer run it, so a 127 about `timeout` is now honestly the user's own command failing.
