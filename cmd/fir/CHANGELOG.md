...
- Model picker highlights free models (zero pricing on all cost axes) with a green `[FREE]` badge and coloured model ID, and sorts them ahead of paid duplicates. When two entries share the same provider + display name — the common Poe case where the same underlying model is exposed as both a paid bot and a free bot — the free variant is listed first so `/model` selects it by default. Also applied as a final tiebreaker after SWE-score and provider ordering.
