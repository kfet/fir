...
func TestApplySWEScores(t *testing.T) {
	...
	if result[2].SWEScore != 80.0 {
		t.Errorf("gpt-5.2: SWEScore = %v, want 80.0", result[2].SWEScore)
	}
...
func TestApplySWEScores_LiveOverride(t *testing.T) {
	...
	// Simulate live leaderboard with a higher score.
	...
	if result[0].SWEScore != 85.0 {
		t.Errorf("claude-opus-4-5 with live override: SWEScore = %v, want 85.0", result[0].SWEScore)
	}
}
