...
- **Batch tool** — the `batch` builtin extension provides a `batch_run` tool and `/batch` slash command. It uses `ctx.call_tool()` + `ctx.side_query()` to orchestrate multi-tool workflows from Python with the same ephemeral pattern. Raw tool outputs never enter the main conversation — only the synthesis is returned. Use `/batch <description>` to ask the agent to build and run a batch automatically.
