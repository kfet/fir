...
- **`rexec(host, command, timeout_s, cwd, detach)`** — run a shell script on a remote host. The script is shipped on **ssh stdin**, never as `ssh host "bash -lc '...'"`, so nothing is re-split by an intermediate shell and no escaping is needed; it is staged to a temp file over there and run as `bash -l <file>`. The `timeout_s` bound is a self-contained bash wrapper travelling in the same payload (own process group, TERM then KILL, exit 124) — **not** GNU `timeout`, which does not exist on macOS. `detach=True` runs it under `systemd-run --user --collect` (fallbacks: double-forked `setsid`, then `set -m` + `nohup` for macOS) on the remote box and returns a job id.
