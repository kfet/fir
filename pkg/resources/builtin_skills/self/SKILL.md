...
- **Duplicate installs**: both install paths warn when more than one `fir`
  binary exists. `install.sh` scans every PATH entry plus the well-known spots
  (`/usr/local/bin`, `~/.local/bin`, `~/go/bin`, `~/bin`, `brew --prefix`/bin)
  and lists each distinct binary with its version, marking the one PATH
  resolves; the Homebrew formula's `caveats` prints the same warning when a
  non-Homebrew `fir` is on PATH, and stays silent otherwise. `FIR_NO_UPDATE_CHECK=1`
  makes `fir --version` (and every background update check) skip the network —
  that is how those scans stay fast.
