...
- **Homebrew** (macOS, Linux): `brew install kfet/fir/fir` (auto-taps `kfet/homebrew-fir`; the formula is regenerated by GoReleaser on each release and pulls binaries from the `kfet/fir-dist` mirror, so no GitHub auth is required).
