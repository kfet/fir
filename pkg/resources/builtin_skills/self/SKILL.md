...
- **MCP authentication (OAuth)** — remote (`streamable`/`sse`) MCP servers need **no configuration** to authenticate. fir connects unauthenticated; on a `401` it runs the MCP authorization-spec discovery chain automatically — RFC 9728 protected-resource metadata (from the `WWW-Authenticate` challenge, falling back to the well-known path), RFC 8414 authorization-server metadata, RFC 7591 dynamic client registration — then an authorization-code + PKCE login with the RFC 8707 `resource` indicator. Tokens are stored in `auth.json` under an `mcp:<server>` key, bound to the server URL they were minted for, refreshed proactively before expiry and silently on a 401.
  ...
  - Optional `auth` overrides (all absent by default — a spec-compliant server needs none):
    ...
    ```
    `mode` is `oauth` (force login before the first request), `bearer` (static token; inferred when `token` is set), or `none` (opt out entirely — a 401 surfaces verbatim). `token` accepts `${VAR}`/`$VAR` so a secret need not be written to disk. `client_id`/`client_secret` cover authorization servers without dynamic registration. `scopes` overrides the scope fir requests (otherwise: challenge `scope`, then `scopes_supported`, then no `scope` parameter at all). `authorization_servers` forces the OAuth issuer(s) for a server whose protected-resource metadata is absent, wrong or unreachable — it *replaces* the advertised list (tried in order, https-or-loopback, no query/fragment), makes a failed metadata fetch non-fatal, and invalidates any stored token minted by an issuer no longer on the list; it is rejected in `bearer`/`none` mode, where the OAuth chain never runs. `redirect_uri` pins the loopback redirect URI for authorization servers that require an exactly pre-registered value and ignore RFC 8252 §7.3 port variance (Okta) — used verbatim (`localhost` ≠ `127.0.0.1`), must be `http` on a loopback host with an explicit non-zero port and a path and no query/fragment, binds that exact host:port at login, normally paired with `client_id`, and likewise rejected in `bearer`/`none` mode. `allow_private_network` lets the OAuth discovery and token legs reach an authorization server on a private or otherwise non-public address — the MCP Go SDK installs a hardened discovery transport that refuses such dials by default, and fir opts out the documented way, by handing the SDK an explicit dialer (`oauthex` `newDiscoveryClient` treats a custom `DialContext` as the signal to stand down); it does *not* unblock private IP **literals** (`https://192.168.1.10/…` is rejected by the SDK at URL level with no opt-out), so what it enables is a private DNS name with a real certificate (`https://mcp.corp.internal/…`), and it does *not* relax fir's own plaintext rule — credentials still never ride `http` to a non-loopback host, for which the escape hatch remains `mode: none` — and, like the other OAuth-only fields, it is meaningful only where the resolved mode runs the OAuth chain (the default, or `oauth`), being rejected in `bearer`/`none` mode.
