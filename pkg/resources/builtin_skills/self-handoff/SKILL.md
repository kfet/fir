...
2. Use `tmux send-keys` to queue `/new` + the handoff instruction into
   your own tmux pane
3. The bash tool returns. fir processes the queued `/new`, clears the
   session. The handoff instruction becomes the first prompt in the
   clean session.
...
- **Temporary** (`/tmp/fir-handoff-<timestamp>.md`): for quick task
...
After writing the handoff doc, run ONE bash command:
...
`tmux send-keys` is synchronous and instant — it writes to tmux's
input buffer and returns in microseconds. Both calls complete before
fir could possibly cancel the bash tool. fir then processes the queued
`/new` (clears session) and the follow-up message becomes the first
prompt in the fresh context.
...
- Keep the follow-up message short — just point to the doc. Don't
  inline the handoff content in the tmux command.
