...
func TestModelRegistry_RegisterProvider(t *testing.T) {
	...
	if m.Name != "Extension Model 1" {
		t.Errorf("expected 'Extension Model 1', got %q", m.Name)
	}
	if m.BaseURL != "https://ext.example.com" {
		t.Errorf("expected baseURL, got %q", m.BaseURL)
	}
}

func TestModelRegistry_RegisterProvider_OverrideOnly(t *testing.T) {
	ai.RegisterModel(&ai.Model{
		ID:            "proxy-target",
		Name:          "Proxy Target",
		Api:           ai.ApiOpenAICompletions,
		Provider:      "test-provider-proxy",
		BaseURL:       "https://api.original.com",
		Input:         []ai.InputModality{ai.InputText},
		Cost:          ai.ModelCost{},
		ContextWindow: 100000,
	...
	if m == nil {
		t.Fatal("model not found after override")
	}
	if m.BaseURL != "https://proxy.example.com" {
		t.Errorf("expected proxied baseURL, got %q", m.BaseURL)
	}
}
