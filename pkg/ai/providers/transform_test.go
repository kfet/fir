...
func TestTransformMessages_RedactedThinking_SameModel(t *testing.T) {
	model := &ai.Model{ID: "claude-opus-4-6", Provider: ai.ProviderAnthropic, Api: ai.ApiAnthropicMessages}
	msg := ai.AssistantMessage{
		Provider: ai.ProviderAnthropic,
		Api:      ai.ApiAnthropicMessages,
		Model:    "claude-opus-4-6",
		Content: []ai.AssistantContent{
			{Thinking: &ai.ThinkingContent{Type: ai.ContentTypeThinking, Thinking: "[Reasoning redacted]", ThinkingSignature: "encrypted-data", Redacted: true}},
			ai.NewTextContent("answer"),
		},
		StopReason: ai.StopReasonStop,
	}
	...
	if !am.Content[0].IsThinking() {
		t.Fatal("expected thinking block kept for same model")
	}
	if !am.Content[0].Thinking.Redacted {
		t.Error("expected Redacted=true preserved")
	}
}

func TestTransformMessages_RedactedThinking_DifferentModel(t *testing.T) {
	model := &ai.Model{ID: "gpt-4", Provider: ai.ProviderOpenAI, Api: ai.ApiOpenAICompletions}
	msg := ai.AssistantMessage{
		Provider: ai.ProviderAnthropic,
		Api:      ai.ApiAnthropicMessages,
	...
	am := result[0].AsAssistant()
	if len(am.Content) != 1 {
		t.Fatalf("expected 1 content block (redacted dropped), got %d", len(am.Content))
	}
	if !am.Content[0].IsText() {
		t.Error("expected only text block remaining")
	}
}
