...
func newTreeList(tree []*store.SessionTreeNode, currentLeafID string) *TreeList {
	tl := &TreeList{
...
func (tl *TreeList) flattenTree(roots []*store.SessionTreeNode) []flatNode {
	var result []flatNode
	...
	containsActive := make(map[*store.SessionTreeNode]bool)
	{
		var allNodes []*store.SessionTreeNode
		stack := make([]*store.SessionTreeNode, len(roots))
		copy(stack, roots)
		for len(stack) > 0 {
	...
	orderedRoots := make([]*store.SessionTreeNode, len(roots))
	...
	for len(stack) > 0 {
		...
		orderedChildren := make([]*store.SessionTreeNode, len(children))
...
func sortByActive(nodes []*store.SessionTreeNode, containsActive map[*store.SessionTreeNode]bool) {
	// Stable sort: active first, then original order
	prioritized := make([]*store.SessionTreeNode, 0, len(nodes))
	rest := make([]*store.SessionTreeNode, 0, len(nodes))
	for _, n := range nodes {
		if containsActive[n] {
...
func (tl *TreeList) extractToolCalls(entry *store.SessionEntry) {
...
func (tl *TreeList) parseMessageInfo(entry *store.SessionEntry) (role string, hasText bool, stopReason string, errorMessage string) {
...
func (tl *TreeList) getSearchableText(node *store.SessionTreeNode) string {
	entry := node.Entry
	var parts []string
	if node.Label != "" {
...
func (tl *TreeList) getEntryDisplayText(node *store.SessionTreeNode, isSelected bool) string {
	t := theme.GetTheme()
	entry := node.Entry
...
func (tl *TreeList) parseToolResult(entry *store.SessionEntry) (toolCallID, toolName string) {
	if len(entry.RawMessage) == 0 {
		return "", ""
	}
	var msg struct {
...
func (tl *TreeList) parseBashCommand(entry *store.SessionEntry) string {
	if len(entry.RawMessage) == 0 {
		return ""
	}
	var msg struct {
