...
func getSessionSearchText(session *core.SessionListInfo) string {
	return session.ID + " " + session.Name + " " + session.AllMessagesText + " " + session.Cwd
}

// HasSessionName returns whether a session has a user-defined name.
func HasSessionName(session *core.SessionListInfo) bool {
	return strings.TrimSpace(session.Name) != ""
}

func matchesNameFilter(session *core.SessionListInfo, filter NameFilter) bool {
...
func MatchSession(session *core.SessionListInfo, parsed ParsedSearchQuery) MatchResult {
...
func FilterAndSortSessions(
...
) []*core.SessionListInfo {
	// Apply name filter
	var nameFiltered []*core.SessionListInfo
	...
	if sortMode == SortRecent {
		var filtered []*core.SessionListInfo
	...
	out := make([]*core.SessionListInfo, len(results))
