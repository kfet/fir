...
func makeUserEntry(id, text string) *store.SessionEntry {
...
func makeAssistantEntry(id, text string, usage *ai.Usage) *store.SessionEntry {
...
func makeToolResultEntry(id string) *store.SessionEntry {
...
func TestShouldCompact_MaxContextTokens(t *testing.T) {
	...
	if !ShouldCompact(6000, 100000, settings) {
		t.Error("should compact when MaxContextTokens exceeded")
	}
	if ShouldCompact(4000, 100000, settings) {
		t.Error("should not compact when under MaxContextTokens")
	}
}

// TestShouldCompact_MaxContextTokensZeroDisabled verifies that
// MaxContextTokens==0 disables the absolute token cap.
func TestShouldCompact_MaxContextTokensZeroDisabled(t *testing.T) {
	settings := CompactionSettings{Enabled: true, ReserveTokens: 1000}
	if ShouldCompact(50000, 100000, settings) {
		t.Error("MaxContextTokens=0 should not trigger token cap")
	}
}

// TestShouldCompact_NegativeMaxContextTokens verifies that a negative
// MaxContextTokens is treated as disabled.
func TestShouldCompact_NegativeMaxContextTokens(t *testing.T) {
	settings := CompactionSettings{
		Enabled:          true,
		ReserveTokens:    1000,
	...
	if ShouldCompact(5000, 10000, settings) {
		t.Error("negative MaxContextTokens should be treated as disabled")
	}
}

// TestShouldCompact_DefaultBehaviorPreserved verifies that without
// MaxContextTokens set, the existing fill-ratio + reserve behavior is unchanged.
func TestShouldCompact_DefaultBehaviorPreserved(t *testing.T) {
	base := CompactionSettings{Enabled: true, ReserveTokens: 1000}
	if ShouldCompact(5000, 10000, base) {
		t.Error("should not compact at 50% fill")
	}
	if !ShouldCompact(9500, 10000, base) {
		t.Error("should compact at 95% fill")
	}
}
...
func TestGenerateSummary_ProgressCallback(t *testing.T) {
	...
	var phases []string
	var deltas []string
	progressFn := session.CompactionProgressFunc(func(phase, delta string) {
