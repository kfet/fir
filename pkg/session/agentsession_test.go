...
// ============================================================================
// trimMessagesForSideQuery
// ============================================================================

func TestTrimMessagesForSideQuery_SnapsToUserBoundary(t *testing.T) {
	// Simulate: user, assistant (with tool_use), tool_result, user, assistant
	// Force trimming so it drops the first user msg, then must snap past
	// the orphaned assistant+tool_result to land on the second user.
	msgs := []agent.AgentMessage{
		agent.NewAgentMessage(ai.NewUserMsg(strings.Repeat("x", 400), 0)),
		agent.NewAgentMessage(ai.NewAssistantMsg(ai.AssistantMessage{
			Role: ai.RoleAssistant,
			Content: []ai.AssistantContent{
				{Text: &ai.TextContent{Text: strings.Repeat("y", 400)}},
				{ToolCall: &ai.ToolCall{ID: "tc1", Name: "bash", Arguments: map[string]any{"command": "ls"}}},
			},
		})),
		agent.NewAgentMessage(ai.NewToolResultMsg(ai.ToolResultMessage{
			...
			ToolCallID: "tc1",
			ToolName:   "bash",
			Content:    []ai.ToolResultContent{{Type: "text", Text: strings.Repeat("z", 400)}},
		})),
		agent.NewAgentMessage(ai.NewUserMsg("second question", 0)),
		agent.NewAgentMessage(ai.NewAssistantMsg(ai.AssistantMessage{
			Role:    ai.RoleAssistant,
			Content: []ai.AssistantContent{{Text: &ai.TextContent{Text: "answer"}}},
		})),
	}

	// contextWindow=400 → budget=280.  Total ≈ 305 tokens (1221 chars / 4).
	// Trimming drops msg[0] (100 tokens), landing on msg[1] (assistant).
	// Snap must advance past assistant+tool_result to msg[3] (second user).
	result := trimMessagesForSideQuery(msgs, 400, 0)
	...
	// Verify it's the second user message.
	u := result[0].Message.AsUser()
	if u == nil {
		t.Fatal("expected user message")
	}
	if content, ok := u.Content.(string); ok && content != "second question" {
		t.Errorf("expected 'second question', got %q", content)
	}
}

func TestTrimMessagesForSideQuery_NoTrimNeeded(t *testing.T) {
	msgs := []agent.AgentMessage{
		agent.NewAgentMessage(ai.NewUserMsg("hello", 0)),
		agent.NewAgentMessage(ai.NewAssistantMsg(ai.AssistantMessage{
			Role:    ai.RoleAssistant,
	...
	result := trimMessagesForSideQuery(msgs, 100000, 0)
	if len(result) != len(msgs) {
		t.Errorf("expected %d messages, got %d", len(msgs), len(result))
	}
}

func TestTrimMessagesForSideQuery_AllToolResults(t *testing.T) {
	// Edge case: all messages except the last are tool_results.
	// Should keep at least the last message.
	msgs := []agent.AgentMessage{
		agent.NewAgentMessage(ai.NewToolResultMsg(ai.ToolResultMessage{
			Role: ai.RoleToolResult, ToolCallID: "tc1", ToolName: "bash",
			Content: []ai.ToolResultContent{{Type: "text", Text: "output"}},
		})),
		agent.NewAgentMessage(ai.NewToolResultMsg(ai.ToolResultMessage{
			Role: ai.RoleToolResult, ToolCallID: "tc2", ToolName: "read",
			Content: []ai.ToolResultContent{{Type: "text", Text: "file contents"}},
		})),
		agent.NewAgentMessage(ai.NewUserMsg("final question", 0)),
	}
	result := trimMessagesForSideQuery(msgs, 10, 90)
	if len(result) == 0 {
		t.Fatal("expected non-empty result")
	}
	if result[0].Role() != "user" {
