...
func TestFormatPlanMarkdown_FirstUpdate(t *testing.T) {
	...
	// First update should NOT be wrapped in <details>
	if strings.Contains(got, "<details>") {
		t.Error("first update should not be collapsible")
	}
	...
	if !strings.Contains(got, "✅ Build binary") {
		t.Error("should show completed entry with checkmark")
	}
	if !strings.Contains(got, "🔄 Run tests") {
		t.Error("should show in_progress entry")
	}
	if !strings.Contains(got, "⬜ Push to prod") {
		t.Error("should show pending entry")
	}
	if !strings.Contains(got, "❗") {
		t.Error("high priority should show exclamation mark")
	}
...
func TestFormatPlanMarkdown_SubsequentUpdate_Collapsible(t *testing.T) {
	args := map[string]any{
		"title": "Deploy service",
		"entries": []any{
	...
	if !strings.Contains(got, "<details>") {
		t.Error("subsequent update should be collapsible")
	}
	if !strings.Contains(got, "</details>") {
		t.Error("should close details tag")
	}
	if !strings.Contains(got, "<summary>") {
		t.Error("should have summary tag")
	}
	if !strings.Contains(got, "2/3") {
		t.Error("summary should show completion count")
	}
}
...
func TestAutoReply_PlanTool_RichRendering(t *testing.T) {
	...
	if !strings.Contains(text, "✅ Step A") {
		t.Errorf("should contain completed step, got: %s", text)
	}
	if !strings.Contains(text, "🔄 Step B") {
		t.Errorf("should contain in_progress step, got: %s", text)
	}
	// First update — should NOT be collapsible
	if strings.Contains(text, "<details>") {
		t.Error("first plan update should not be collapsible")
	}
}
...
func TestAutoReply_PlanTool_SecondUpdate_Collapsible(t *testing.T) {
	...
	// Second call should be collapsible
	text := calls[1]["text"].(string)
	if !strings.Contains(text, "<details>") {
		t.Error("second plan update should be collapsible")
	}
}
