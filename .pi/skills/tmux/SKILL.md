...
license: Vibecoded
reference: "https://github.com/mitsuhiko/agent-stuff/tree/main/skills/tmux"
---
...
- List sessions on your active socket with metadata: `./.pi/skills/tmux/scripts/find-sessions.sh -S "$SOCKET"`; add `-q partial-name` to filter.
- Scan all sockets under the shared directory: `./.pi/skills/tmux/scripts/find-sessions.sh --all` (uses `CLAUDE_TMUX_SOCKET_DIR` or `${TMPDIR:-/tmp}/claude-tmux-sockets`).
...
- Use timed polling to avoid races with interactive tools. Example: wait for a Python prompt before sending code:
  ```bash
  ./.pi/skills/tmux/scripts/wait-for-text.sh -t "$SESSION":0.0 -p '^>>>' -T 15 -l 4000
  ```
...
The `.pi/skills/tmux/scripts/` directory contains helper tools:

### wait-for-text.sh

`./.pi/skills/tmux/scripts/wait-for-text.sh` polls a pane for a regex (or fixed string) with a timeout. Works on Linux/macOS with bash + tmux + grep.

```bash
./.pi/skills/tmux/scripts/wait-for-text.sh -t session:0.0 -p 'pattern' [-F] [-T 20] [-i 0.5] [-l 2000]
```
...
`./.pi/skills/tmux/scripts/find-sessions.sh` lists tmux sessions on a socket with metadata.

```bash
./.pi/skills/tmux/scripts/find-sessions.sh [-L socket-name|-S socket-path|-A] [-q pattern]
```
